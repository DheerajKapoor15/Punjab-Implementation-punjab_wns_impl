CREATE OR REPLACE FUNCTION wtms_bill_genaration_for_non_metered_connections(batch_name varchar,boundary_type varchar,curr_installment_description varchar)
RETURNS void AS
$BODY$
DECLARE
props record;
...
raise notice 'v_curr_inst_id,v_curr_inst_id_temp % %',v_curr_inst_id,v_curr_inst_id_temp;
	...
	if(v_curr_inst_id != v_curr_inst_id_temp) then
		...
		loop 
			...
			v_temp := wtms_bill_genaration_for_location(boundary_type,props.id,v_curr_inst_id,v_prev_inst_id,curr_installment_description);
...
create or replace function wtms_bill_genaration_for_location(boundary_type varchar,in_location_id bigint,in_curr_inst_id bigint,in_prev_inst_id bigint,curr_installment_description varchar)
returns numeric as 
$BODY$
declare
rec record;
	  	...
	  	loop
			begin
				raise notice 'Started bill generation for consumercode: %',rec.consumercode;
	    			...
	    			v_temp := wtms_bill_genaration_for_consumer_code(rec.consumercode,rec.connid,rec.conndetid, in_curr_inst_id, in_prev_inst_id,curr_installment_description,rec.locationid);
...
create or replace function wtms_bill_genaration_for_consumer_code(in_consumercode varchar(16), in_conn_id bigint,in_conn_det_id bigint,in_curr_inst_id bigint,in_prev_inst_id bigint,curr_installment_description varchar,in_location_id bigint)
returns numeric  as 
$BODY$
declare
...
begin
	...
	if(v_currdemand is not null) then
		...
		if(v_demandbill is null) then
			...
			v_penalty := wtms_penalty_and_interest_generate_for_consumercode(in_consumercode, in_curr_inst_id,in_prev_inst_id);

DROP FUNCTION IF EXISTS stms_rates_by_connectiondetails_id(app_detail_id bigint, in_curr_instl_end_date date);
...
BEGIN
	...
	if(v_wateridentifier is not null) then
		...
		select id into v_wt_chargers_demandreason from eg_demand_reason where id_demand_reason_master in (select id from eg_demand_reason_master where code='WTAXCHARGES' and module = (select id from eg_module where name='Water Tax Management')) and id_installment=in_curr_inst_id;
		select d.id into v_currdemandid from eg_demand d, egwtr_demand_connection dc where dc.connectiondetails=v_wt_conn_det_id and dc.demand=d.id and d.id_installment=in_curr_inst_id;
		...
		if(v_connectiontype is not null and v_connectiontype = 'METERED') then
			select meterstatus into v_meterstatus from egwtr_meter_reading_details where connectiondetails = v_wt_conn_det_id and installment=in_curr_inst_id;
		end if;
	end if;

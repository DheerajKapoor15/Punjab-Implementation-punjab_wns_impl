...
BEGIN
	...
	if(v_curr_inst_id != v_curr_inst_id_temp) then
		...
		loop 
			...
			v_temp := stms_bill_genaration_for_boundary(boundary_type,props.id,v_curr_inst_id,v_prev_inst_id,curr_installment_description);
...
drop function stms_bill_genaration_for_location(boundary_type varchar,in_location_id bigint,in_curr_inst_id bigint,in_prev_inst_id bigint,curr_installment_description varchar);
--procedure for Add eg_bill and eg_bill_details entry
create or replace function stms_bill_genaration_for_boundary(boundary_type varchar,in_location_id bigint,in_curr_inst_id bigint,in_prev_inst_id bigint,curr_installment_description varchar)
returns numeric as 
$BODY$
declare
rec record;

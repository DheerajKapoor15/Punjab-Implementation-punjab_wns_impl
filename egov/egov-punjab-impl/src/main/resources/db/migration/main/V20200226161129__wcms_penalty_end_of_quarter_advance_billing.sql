...
select id into billinstallment  from eg_installment_master where start_date <=now() and end_date>=now() and id_module=(select id from eg_module where name='Water Tax Management');
			...
			insert into eg_bill(id, id_demand, citizen_name, citizen_address, bill_no, id_bill_type, issue_date, last_date, module_id, user_id, 
			create_date, modified_date, is_history, is_cancelled, fundcode, functionary_code, fundsource_code, department_code, coll_modes_not_allowed, 
			boundary_num, boundary_type, total_amount, total_collected_amount, service_code, part_payment_allowed, override_accountheads_allowed, 
			description, min_amt_payable, consumer_id, dspl_message, callback_for_apportion, penalty) values (v_billpk, v_currdemand, v_ownername, 
			v_houseno, v_billno, v_idbillype, now(), now()+INTERVAL '1 DAY'*v_penalty_period, v_moduleid, v_iduser, now(), now(), 'N', 'N', v_fund_code, v_functionary_code, v_fundsource_code, v_dept_code, null, in_location_id, null, v_totalamount, v_totalamountcollected, 'WT', 'Y', 'N', v_description, 0, in_consumercode, 'Water Charge Collection', 'N', v_penalty);

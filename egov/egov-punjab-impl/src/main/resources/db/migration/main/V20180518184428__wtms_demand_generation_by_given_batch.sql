DROP FUNCTION wtms_demand_genaration_for_non_metered_connections(character varying,character varying);

CREATE OR REPLACE FUNCTION wtms_demand_genaration_for_non_metered_connections(batch_name varchar,curr_installment_description varchar)
RETURNS void AS
$BODY$
DECLARE
props record;
...
BEGIN
	...
	select b.id into v_prev_bill_id from eg_bill b,eg_demand d where b.id_demand = d.id and d.id_installment = in_prev_inst_id;

...
BEGIN
	...
	select inst.id into v_connlatestinstid from eg_demand d, egswtax_demand_connection dc, eg_installment_master inst where dc.applicationdetail=in_idappdetail and dc.demand=d.id and inst.id=d.id_installment order by inst.start_date desc limit 1;
	...
	if(v_currdemandid is null) then
		if((v_olddemandid is not null and v_prev_bill_id is not null) or (v_olddemandid is null)) then
			...
			if(v_demandAmountfromSWRate is not null and v_demandAmountfromSWRate > 0) then
				...
				raise exception 'No Sewerage Rates present for cousumer code: %',in_consumercode ;
			end if;
			else

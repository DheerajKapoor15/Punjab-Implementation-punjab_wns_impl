...
--Sewerage integration
		...
		if (in_bill_type= 'INTEGRATED') then
			...
			v_temp := stms_bill_detail_genaration_for_water(v_stms_appdetid, v_billpk, curr_installment_description, in_bill_type);
	  		...
	  		raise exception 'Bill allready present for this cosumer code (%)',in_consumercode;
	  	end if;
	else
		raise exception 'Demand does not exist for current installment %', in_consumercode;
	end if;

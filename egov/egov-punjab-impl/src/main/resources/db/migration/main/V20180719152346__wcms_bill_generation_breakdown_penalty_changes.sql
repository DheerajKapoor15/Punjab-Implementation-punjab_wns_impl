DROP FUNCTION IF EXISTS wtms_penalty_and_interest_generate_for_consumercode(in_consumercode varchar(16), in_curr_inst_id bigint,in_prev_inst_id bigint, in_bill_type varchar);

DROP FUNCTION IF EXISTS wtms_bill_detail_genaration_for_consumer_code(v_billpk bigint, in_demandid bigint,v_olddemand bigint, in_curr_inst_id bigint,in_prev_inst_id bigint,curr_installment_description varchar);

CREATE OR REPLACE FUNCTION wtms_bill_genaration(connection_type varchar,batch_name varchar,boundary_type varchar,curr_installment_description varchar, batch_id bigint)
RETURNS void AS
$BODY$
DECLARE
props record;
...
create or replace function wtms_bill_genaration_for_boundary(connection_type varchar,boundary_type varchar, in_location_id bigint, in_curr_inst_id bigint, in_prev_inst_id bigint, curr_installment_description varchar, batch_id bigint)
returns numeric as 
$BODY$
declare
rec record;
...
begin
	...
	if(v_currdemand is not null) then
		...
		if(v_demandbill is null) then
			...
			v_penalty := wtms_penalty_and_interest_generate_for_consumercode(v_moduleid,v_idbillype,in_conn_det_id,in_consumercode,
v_currdemand,v_olddemand,in_curr_inst_id,in_prev_inst_id);
...
--Sewerage integration
		...
		if (in_bill_type= 'INTEGRATED') then
			...
			if (batch_id is null or batch_id = 0) then
					raise exception 'Bill allready present for this consumer number: %',in_consumercode ;			else
				v_status := false;
				insert into egwtr_batchjobdetailslog(id, jobdetails, consumercode, status, reason, createddate, lastmodifieddate, createdby, lastmodifiedby) values(nextval('seq_egwtr_batchjobdetailslog'), batch_id, in_consumercode, 'Failure', 'Bill allready present for this consumer number: ' || in_consumercode, now(), now(), (select id from eg_user where username='egovernments'), (select id from eg_user where username='egovernments'));
			end if;
	  	end if;
	else
		if (batch_id is null or batch_id = 0) then
...
begin
	v_penalty := 0;
	select issue_date into v_billdate from eg_bill where consumer_id=in_consumercode and module_id = in_moduleid and id_bill_type = in_idbillype and is_cancelled = 'N' and id_demand=in_previousdemandid;

DROP FUNCTION IF EXISTS wtms_rates_by_connectiondetails_id_for_metered(conn_detail_id bigint,in_curr_inst_id bigint,in_curr_instl_end_date date);
...
BEGIN
	...
	select billingType, connectionType, usageType, averageMeterReading into v_billingtype, v_connectiontype, v_usagetype, v_averageMeterReading from egwtr_connectiondetails where id = conn_detail_id;
	...
	if(v_connectiontype is not null and v_connectiontype = v_connectiontypeid and v_billingtype is not null and v_billingtype = 'STANDARD' ) then
		...
		if((v_unitsconsumed is not null and v_unitsconsumed > 0 ) or v_meterstatus = 'Locked' or v_meterstatus = 'Breakdown' or v_meterstatus = 'No_Meter') then
			...
			if (batch_id is null or batch_id = 0) then
				...
				insert into egwtr_batchjobdetailslog(id, jobdetails, consumercode, status, reason, createddate, lastmodifieddate, createdby, lastmodifiedby) values(nextval('seq_egwtr_batchjobdetailslog'), batch_id, null, 'Failure', 'Meter reading entry not present for selected Billing Cycle', now(), now(), (select id from eg_user where username='egovernments'), (select id from eg_user where username='egovernments'));
			end if;

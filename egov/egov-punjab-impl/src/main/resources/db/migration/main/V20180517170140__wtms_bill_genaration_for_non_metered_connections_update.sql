...
'1806006', '50515100', (dd.amount - dd.amt_collected), 0, 'Water Charges -'::character varying|| install.description, install.id, 1,'ARREAR_AMOUNT' from eg_demand_details dd,eg_installment_master install,eg_demand_reason demreas where  dd.id_demand_reason=demreas.id and demreas.id_installment=install.id 
and id_demand_reason not in(select id from eg_demand_reason where id_demand_reason_master in (select id from eg_demand_reason_master where code='WTAXCHARGES' and module=(select id from eg_module where name='Water Tax Management')) and id_installment in (in_curr_inst_id)) and dd.id_demand=in_demandid and dd.amount>0 and  dd.amount != dd.amt_collected;
	...
	insert into eg_bill_details(id, id_demand_reason, create_date, modified_date,id_bill, collected_amount, order_no, glcode, function_code, cr_amount, dr_amount, description, id_installment, additional_flag,purpose) select nextval('seq_eg_bill_details'), dd.id_demand_reason, now(), now(), v_billpk, 0, 2, 
'1806006', '50515100', (dd.amount - dd.amt_collected), 0, 'Water Charges -'::character varying|| install.description, install.id, 1,'PENALTY' from eg_demand_details dd,eg_installment_master install,eg_demand_reason demreas where  dd.id_demand_reason=demreas.id and demreas.id_installment=install.id 
and id_demand_reason not in(select id from eg_demand_reason where id_demand_reason_master in (select id from eg_demand_reason_master where code='PENALTY' and module=(select id from eg_module where name='Water Tax Management')) and id_installment in (in_curr_inst_id)) and dd.id_demand=in_demandid and dd.amount>0 and  dd.amount != dd.amt_collected;

	insert into eg_bill_details(id, id_demand_reason, create_date, modified_date,id_bill, collected_amount, order_no, glcode, function_code, cr_amount, dr_amount, description, id_installment, additional_flag,purpose) select nextval('seq_eg_bill_details'), dd.id_demand_reason, now(), now(), v_billpk, 0, 2, 
'1806006', '50515100', (dd.amount - dd.amt_collected), 0, 'Water Charges -'::character varying|| install.description, install.id, 1,'INTEREST' from eg_demand_details dd,eg_installment_master install,eg_demand_reason demreas where  dd.id_demand_reason=demreas.id and demreas.id_installment=install.id 
and id_demand_reason not in(select id from eg_demand_reason where id_demand_reason_master in (select id from eg_demand_reason_master where code='INTEREST' and module=(select id from eg_module where name='Water Tax Management')) and id_installment in (in_curr_inst_id)) and dd.id_demand=in_demandid and dd.amount>0 and  dd.amount != dd.amt_collected;
...
--TODO : bill has to be generated for every billing cycle, do we need a check for this?
begin
	select id into v_moduleid from eg_module where name ='Water Tax Management';
	select id into v_idbillype from  eg_bill_type where name='MANUAL';
	...
	select dem.id into v_currdemandid from eg_demand dem, egwtr_demand_connection conn where dem.id=dd.id_demand and conn.connectiondetails=v_conn_det_id and dem.id_installment=in_curr_inst_id;
	select dem.id into v_previousdemandid from eg_demand dem, egwtr_demand_connection conn where dem.id=dd.id_demand and conn.connectiondetails=v_conn_det_id and dem.id_installment=in_prev_inst_id;
	...
	if(v_billdate is not null) then
		...
		select 	v_billdate + INTERVAL '30 DAY' into v_penalty_date;
		select 	v_billdate + INTERVAL '60 DAY' into v_interest_date;
		...
		if(v_penalty_date < now() and v_previousChargesBal>0) then
			...
			v_penalty := (v_previousChargesBal * 10)/100;
		...
		if(v_interest_date < now() and v_previousChargesBal>0) then
			...
			v_interest := (v_previousChargesBal * v_interest_months)/100;
		...
		raise notice 'Bill does not exist for previous installment %',v_consumercode;
	end if;

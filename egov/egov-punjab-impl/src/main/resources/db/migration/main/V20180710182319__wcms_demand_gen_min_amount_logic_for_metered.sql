...
BEGIN
	...
	if(v_connectiontype is not null and v_connectiontype = 'METERED' and v_billingtype is not null and v_billingtype = 'STANDARD' ) then
		...
		if(v_unitsconsumed is not null and v_unitsconsumed > 0) then
			select rateAmount,minimumamount into v_rate,v_minAmount from egwtr_watermeteredcharge_rates where usagetype = v_usagetype and effectiveFromDate<=in_curr_instl_end_date and effectiveToDate>=in_curr_instl_end_date and ((fromVolume<= v_unitsconsumed and toVolume >= v_unitsconsumed) or (fromVolume<= v_unitsconsumed and toVolume is null));
			if(v_minAmount is not null and v_minAmount > 0) then
				v_rateAmount = v_minAmount;
			else
				select minimumamount,toVolume into v_minAmount,v_toVolume from egwtr_watermeteredcharge_rates where usagetype = v_usagetype and effectiveFromDate<=in_curr_instl_end_date and effectiveToDate>=in_curr_instl_end_date and fromVolume < v_unitsconsumed and toVolume < v_unitsconsumed and minimumamount is not null and minimumamount > 0 ;
				v_diff = v_unitsconsumed - v_toVolume;
				v_rateAmount = v_minAmount + (v_diff * v_rate);
			end if;
		else
			v_rateAmount =null;
		end if;

DROP FUNCTION if exists stms_bill_genaration(varchar,varchar,varchar,bigint);
DROP FUNCTION if exists stms_bill_genaration_for_consumer_code(varchar,bigint,bigint,bigint,varchar,bigint,bigint,bigint);
DROP FUNCTION if exists stms_penalty_and_interest_generate_for_consumercode(varchar,bigint,bigint);
...
v_dept_code character varying(50);
...
begin
	...
	select id into v_currdemand from eg_demand where is_history='N' and id in (select demand from egswtax_demand_connection where applicationdetail=in_appdet_id) and id_installment = in_curr_inst_id;
	...
	if(v_currdemand is not null) then
		...
		select bill.id into v_water_intgbill from eg_bill bill, egwtr_connection wcon, egswtax_connection scon, egwtr_connectiondetails wcdet, egwtr_demand_connection wdcn, eg_demand dm, eg_installment_master mst where scon.oldconsumernumber = wcon.sewerageidentifier and wcon.consumercode = bill.consumer_id and bill.id_bill_type = (select id from eg_bill_type where code='INTEGRATED') and wcon.id=wcdet.connection and wcdet.id=wdcn.connectiondetails and wdcn.demand=dm.id and dm.is_history='N' and bill.id_demand=dm.id and dm.id_installment=mst.id and mst.description=v_cur_description and mst.id_module = (select id from eg_module where name='Water Tax Management') and bill.is_cancelled='N' and scon.shsc_number = in_consumercode and bill.module_id = (select id from eg_module where name='Water Tax Management');

		if (v_water_intgbill is null) then
			select bill.id into v_water_intgbill from eg_bill bill, egwtr_connection wcon, egswtax_connection scon, egwtr_connectiondetails wcdet, egwtr_demand_connection wdcn, eg_demand dm, eg_installment_master mst where scon.shsc_number = wcon.sewerageidentifier and wcon.consumercode = bill.consumer_id and bill.id_bill_type = (select id from eg_bill_type where code='INTEGRATED') and wcon.id=wcdet.connection and wcdet.id=wdcn.connectiondetails and wdcn.demand=dm.id and dm.is_history='N' and bill.id_demand=dm.id and dm.id_installment=mst.id and mst.description=v_cur_description and mst.id_module = (select id from eg_module where name='Water Tax Management') and bill.is_cancelled='N' and scon.shsc_number = in_consumercode and bill.module_id = (select id from eg_module where name='Water Tax Management');
		end if;	
		...
		if(v_demandbill is null and v_water_intgbill is null) then
			...
			select value into v_dept_code from eg_appconfig_values where key_id in (select id from eg_appconfig where module in (select id from eg_module where name = 'Sewerage Tax Management') and key_name = 'DEPT_CODE_FOR_GENERATE_BILL');
			...
			select id into billinstallment  from eg_installment_master where start_date <=now() and end_date>=now() and id_module=(select id from eg_module where name='Sewerage Tax Management');
...
begin
	...
	select dem.id into v_previousdemandid from eg_demand dem, egswtax_demand_connection conn where dem.id=conn.demand and conn.applicationdetail=v_applicationdetail_id and dem.id_installment=in_prev_inst_id order by dem.id desc limit 1;
	select issue_date into v_billdate from eg_bill where consumer_id=in_consumercode and module_id = v_moduleid and id_bill_type != (select id from eg_bill_type where code='AUTO') and is_cancelled = 'N' and id_demand=v_previousdemandid;
	select last_date into v_penalty_date from eg_bill where consumer_id=in_consumercode and module_id = v_moduleid and id_bill_type != (select id from eg_bill_type where code='AUTO') and is_cancelled = 'N' and id_demand=v_previousdemandid;

	select sum(dd.amount), sum(dd.amount-dd.amt_collected) into v_previousCharges, v_previousChargesBal from eg_demand_details dd, eg_demand_reason dr, eg_demand_reason_master drm where dd.id_demand=v_currdemandid and dd.id_demand_reason=dr.id and drm.id=dr.id_demand_reason_master and dr.id_installment=in_prev_inst_id and  drm.code='SEWERAGETAX' and drm.module = v_moduleid;
	

	raise notice 'stms_penalty_and_interest_generate_for_consumercode, v_billdate, v_previousCharges, v_previousChargesBal % % %',v_billdate, v_previousCharges, v_previousChargesBal;
	if(v_billdate is null) then
		select bill.issue_date into v_billdate from eg_bill bill, egwtr_connection wcon, egswtax_connection scon, egwtr_connectiondetails wcdet, egwtr_demand_connection wdcn, eg_demand dm, eg_installment_master mst where scon.oldconsumernumber = wcon.sewerageidentifier and wcon.consumercode = bill.consumer_id and bill.id_bill_type = (select id from eg_bill_type where code='INTEGRATED') and wcon.id=wcdet.connection and wcdet.id=wdcn.connectiondetails and wdcn.demand=dm.id and bill.id_demand=dm.id and dm.id_installment=mst.id and mst.description=(select description from eg_installment_master where id_module=v_moduleid and id=in_prev_inst_id) and mst.id_module = (select id from eg_module where name='Water Tax Management') and bill.is_cancelled='N' and scon.shsc_number = in_consumercode and bill.module_id = (select id from eg_module where name='Water Tax Management');
		select bill.last_date into v_penalty_date from eg_bill bill, egwtr_connection wcon, egswtax_connection scon, egwtr_connectiondetails wcdet, egwtr_demand_connection wdcn, eg_demand dm, eg_installment_master mst where scon.oldconsumernumber = wcon.sewerageidentifier and wcon.consumercode = bill.consumer_id and bill.id_bill_type = (select id from eg_bill_type where code='INTEGRATED') and wcon.id=wcdet.connection and wcdet.id=wdcn.connectiondetails and wdcn.demand=dm.id and bill.id_demand=dm.id and dm.id_installment=mst.id and mst.description=(select description from eg_installment_master where id_module=v_moduleid and id=in_prev_inst_id) and mst.id_module = (select id from eg_module where name='Water Tax Management') and bill.is_cancelled='N' and scon.shsc_number = in_consumercode and bill.module_id = (select id from eg_module where name='Water Tax Management');

		end if;
	
	
	raise notice 'stms_penalty_and_interest_generate_for_consumercode, integratede v_billdate %',v_billdate;
	...
	raise notice 'stms_penalty_and_interest_generate_for_consumercode, v_penalty_period, v_interest_period, v_penalty_percentage, v_interest_percentage, v_penalty_date, v_interest_date % % % % % %',v_penalty_period, v_interest_period, v_penalty_percentage, v_interest_percentage, v_penalty_date, v_interest_date;
		...
		if(v_penalty_date < now() and v_previousChargesBal>0) then
			...
			if(v_penalty_demand_detail is null and v_penalty>0) then

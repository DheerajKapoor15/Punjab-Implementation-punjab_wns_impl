...
--meterserialno 120, metermanufacturer 120, latestreading double, latestreadingdate, meterstatus new var120, avglastthreereading double
	if(v_connectiontype = 'METERED') then
		...
		insert into egwtr_meter_reading_details (id, connectiondetails, installment, currentreading, currentreadingdate, previousreading, previousreadingdate, meterstatus, unitsconsumed, createddate, lastmodifieddate, createdby, lastmodifiedby, version) select nextval('seq_egwtr_meter_reading_details'), v_connectiondetpk, currinst, latestreading, latestreadingdate, null, null, meterstatus, null, connectiondate, connectiondate, 1, 1, 0 from egwtr_stg_connection where oldconsumernumber = oldConnectionNo;
	end if;

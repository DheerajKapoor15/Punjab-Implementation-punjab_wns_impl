...
BEGIN
	begin
		...
		select sum(coalesce(chargeamount, 0) + coalesce(penaltyamount, 0) + coalesce(interestamount, 0)) into v_totaltax from egswtax_stg_connection conn, egswtax_stg_connection_dcb dcb where conn.oldconsumernumber=dcb.oldconsumernumber and conn.oldconsumernumber=oldConnectionNo;

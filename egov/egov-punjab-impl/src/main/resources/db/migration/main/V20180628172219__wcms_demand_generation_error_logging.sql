DROP FUNCTION IF EXISTS wtms_demand_generation_for_consumer_code(in_consumer_code varchar,in_idconndetail bigint, in_curr_inst_id bigint, in_prev_inst_id bigint);

DROP FUNCTION IF EXISTS wtms_demand_genaration_for_non_metered_connections_for_boundary(boundary_type varchar, in_location_id bigint, in_curr_inst_id bigint, in_prev_inst_id bigint);

DROP FUNCTION IF EXISTS wtms_demand_genaration_for_non_metered_connections(batch_name varchar,boundary_type varchar,curr_installment_description varchar);

CREATE OR REPLACE FUNCTION wtms_demand_genaration_for_non_metered_connections(batch_name varchar,boundary_type varchar,curr_installment_description varchar, batch_id bigint)
RETURNS void AS
$BODY$
DECLARE
props record;
...
BEGIN
	...
	loop 
		begin
			...
			v_temp  := wtms_demand_genaration_for_non_metered_connections_for_boundary(boundary_type, props.id, v_curr_inst_id, v_prev_inst_id, batch_id);
		...
		if (batch_id is null) then
			...
			insert into egwtr_batchjobdetailslog(id, jobdetails, consumercode, status, reason, createddate, lastmodifieddate, createdby, lastmodifiedby) values(nextval('seq_egwtr_batchjobdetailslog'), batch_id, null, 'Failure', 'Error in demand generation for ulb ' || props.name || ' ' || SQLERRM || ' ' || SQLSTATE, now(), now(), (select id from eg_user where username='egovernments'), (select id from eg_user where username='egovernments'));
		end if;
		end;
	end loop;
...
CREATE OR REPLACE FUNCTION wtms_demand_genaration_for_non_metered_connections_for_boundary(boundary_type varchar, in_location_id bigint, in_curr_inst_id bigint, in_prev_inst_id bigint, batch_id bigint)
RETURNS numeric AS
$BODY$
DECLARE
props record;
	...
	if(boundary_type is not null and boundary_type = 'Locality') then 
		...
		loop 
			begin
				...
				v_temp  := wtms_demand_generation_for_consumer_code(props.consumercode,props.id, in_curr_inst_id, in_prev_inst_id, batch_id);
...
CREATE OR REPLACE FUNCTION wtms_demand_generation_for_consumer_code(in_consumer_code varchar,in_idconndetail bigint, in_curr_inst_id bigint, in_prev_inst_id bigint, batch_id bigint)
RETURNS numeric AS
$BODY$
DECLARE
	...
	if(v_currdemandid is null) then
		if((v_olddemandid is not null and v_prev_bill_id is not null) or (v_olddemandid is null)) then
			...
			if(v_demandAmountfromWtRate is not null and v_demandAmountfromWtRate > 0) then
				...
				if (batch_id is null) then
					raise exception 'No Water Rates present for cousumer code: %',in_consumer_code ;				else
					v_status := false;
					insert into egwtr_batchjobdetailslog(id, jobdetails, consumercode, status, reason, createddate, lastmodifieddate, createdby, lastmodifiedby) values(nextval('seq_egwtr_batchjobdetailslog'), batch_id, in_consumer_code, 'Failure', 'No Water Rates present for cousumer number: ' || in_consumer_code, now(), now(), (select id from eg_user where username='egovernments'), (select id from eg_user where username='egovernments'));
				end if;
			end if;
		else
			if (batch_id is null) then

...
CREATE OR REPLACE FUNCTION pbwc_migrateCityConnections(in_billingCycle IN varchar(16))
RETURNS bigint AS
$BODY$
DECLARE
	...
	if (v_isdatavalid) then
		...
		loop 
			begin
				...
				v_temp  := pbwc_migrateConnection(connections.oldconsumernumber, connections.connectiondate::date, v_billingcycleid, in_billingCycle, v_moduleid, connections.corraddress);
				if(v_temp=0)then
					v_return:=0;
				end if;
			EXCEPTION
...
CREATE OR REPLACE FUNCTION pbwc_migrateLocalityConnections(localityCode IN varchar(16), in_billingCycle IN varchar(16))
RETURNS bigint AS
$BODY$
DECLARE
connections egwtr_stg_connection%ROWTYPE;
...
BEGIN
	begin
		...
		select count(*) into v_consumercount from egwtr_connection where oldconsumernumber=oldConnectionNo;
		if(v_consumercount>0)then
			...
			update egwtr_stg_connection set error=coalesce(error, '') || ' There is already a connection with given old consumer number' where oldconsumernumber=oldConnectionNo;
			return 0;
		end if;

		select count(*), sum(coalesce(chargeamount, 0) + coalesce(penaltyamount, 0) + coalesce(interestamount, 0)) into v_dcbcount, v_totaltax from egwtr_stg_connection_dcb where oldconsumernumber=oldConnectionNo;
		select count(*) into v_billingmatchingcount from egwtr_stg_connection_dcb where billingcycle=currinstdesc and oldconsumernumber=oldConnectionNo;
		select count(distinct oldconsumernumber) into v_futurebillingcount from egwtr_stg_connection_dcb dcb, eg_installment_master inst where dcb.oldconsumernumber=oldConnectionNo and inst.id_module=idModule and dcb.billingcycle=inst.description and inst.start_date>v_inbillingstartdate;
		...
		if(v_dcbcount>0)then
			if (v_billingmatchingcount=0 or v_futurebillingcount>0) then
				update egwtr_stg_connection set error=coalesce(error, '') || ' There is no DCB with migration billing cycle or there is DCB future billing cycles' where oldconsumernumber=oldConnectionNo;
				return 0;
			end if;
		end if;
		...
		v_idconnectiondet := pbwc_createConnection(oldConnectionNo, v_addressid, currinst);
		--raise notice 'migrateConnection : Connection created v_idconnectiondet (%)', v_idconnectiondet;
		select conn.consumercode, connection into v_newconsumercode, v_idconnection from egwtr_connection conn, egwtr_connectiondetails cd where cd.connection=conn.id and cd.id=v_idconnectiondet;
		...
		--raise notice 'migrateConnection : Ended for Old connectionno (%)', oldConnectionNo;
	...
	update egwtr_stg_connection set error = coalesce(error, '') || SQLSTATE || '-' || SQLERRM where oldconsumernumber=oldConnectionNo;
	END;
END;
$BODY$ LANGUAGE plpgsql;
...
declare
	...
	v_addresspk	bigint;
BEGIN 
	--raise notice 'createOwner oldConnectionNo, corraddr1, corraddr2, addressid (% % %)', oldConnectionNo, corraddress, addressid;
	select nextval('seq_eg_address') into v_addresspk;
	select nextval('seq_eg_user') into v_userpk;
	INSERT INTO eg_user(id, locale, username, password, mobilenumber, emailid, createddate, lastmodifieddate, createdby, lastmodifiedby, active, name, gender, type, guardian, guardianrelation, dob)
    select v_userpk, 'en_IN', pbwc_generateUsername(ownername), 'NOT SET', mobileno, emailid, connectiondate, connectiondate, 1, 1, TRUE, ownername, (CASE WHEN gender = 'Female' THEN 0 ELSE 1 END), 'CITIZEN', guardian, guardianrelation, dob from egwtr_stg_connection where oldconsumernumber = oldConnectionNo;
...
--createConnection
create or replace FUNCTION pbwc_createConnection(oldConnectionNo IN varchar(50), addressid IN bigint, currinst IN bigint)  
RETURNS numeric as $$
declare
...
BEGIN
	...
	select connectiontype into v_connectiontype from egwtr_stg_connection where oldconsumernumber = oldConnectionNo;
...
INSERT INTO  egwtr_connectiondetails (id, connection, applicationdate, executiondate, applicationtype, billingtype, billamount, plotsize, nooftaps, ledgernumber, billgroupnumber, connectiontype, watersource, propertytype, category, usagetype, pipesize, donationcharges, connectionstatus, legacy, statusid, createdby, createddate, lastmodifieddate, lastmodifiedby, approvalnumber, connectionreason, isplotsizeactual, currentoccupier, currentbillingcycle, lastbilldate, disconnectiondate, averagemeterreading, version, ishistory)
	select v_connectiondetpk, v_connectionpk, connectiondate, connectiondate, (select id from egwtr_application_type where code='NEWCONNECTION'), upper(billingtype), custombillamount, plotsize, nooftaps, ledgerid, billgroupid, (select id from egwtr_connection_type where code=connectiontype), (select id from egwtr_water_source where watersourcetype=watersource), (select id from egwtr_property_type where name=propertytype), (select id from egwtr_category where name=category), (select id from egwtr_usage_type where name=usagetype), (select id from egwtr_pipesize where code=pipesize), coalesce(securitydeposit,0), upper(coalesce(status,'ACTIVE')), true, (select id from egw_status where moduletype='WATERTAXAPPLICATION' and code='SANCTIONED'), 1, connectiondate, connectiondate, 1, 'Migrated Connection', 'Migrated Connection', (CASE WHEN isplotsizeactual = 'Yes' THEN TRUE ELSE FALSE END), currentoccupier, currentbillingcycle, lastbilldate, diconnectiondate, avglastthreereading, 0, FALSE from egwtr_stg_connection where oldconsumernumber = oldConnectionNo;

--meterserialno 120, metermanufacturer 120, latestreading double, latestreadingdate, meterstatus new var120, avglastthreereading double
	if(v_connectiontype = 'METERED') then
		raise notice 'createConnection Its metered connection';
		insert into egwtr_meter_details (id, connectiondetails, meterserialnumber, metermake, initialreading, meterexecutiondate, active, createddate, lastmodifieddate, createdby, lastmodifiedby, version) select nextval('seq_egwtr_meter_details'), v_connectiondetpk, meterserialno, metermanufacturer, null, null, true, connectiondate, connectiondate, 1, 1, 0 from egwtr_stg_connection where oldconsumernumber = oldConnectionNo;

		insert into egwtr_meter_reading_details (id, connectiondetails, installment, currentreading, currentreadingdate, meterstatus, unitsconsumed, legacy, createddate, lastmodifieddate, createdby, lastmodifiedby, version) select nextval('seq_egwtr_meter_reading_details'), v_connectiondetpk, currinst, latestreading, latestreadingdate, meterstatus, null, true, connectiondate, connectiondate, 1, 1, 0 from egwtr_stg_connection where oldconsumernumber = oldConnectionNo;
	end if;
...
DECLARE
	...
	v_meteredconnections bigint;
	v_nonmeteredconnections bigint;
	v_totaldcbs bigint;
	v_billingtype bigint;
	...
	v_noofmeterstatusnull bigint;
	v_noofinvalidmeterstatus bigint;

	v_duplicatconnection bigint;
	v_wrongdcbcount bigint;
	v_duplicatebillingcycle bigint;
...
BEGIN
	...
	select count(*) into v_nonmeteredconnections from egwtr_stg_connection where connectiontype='NON_METERED';
	select count(*) into v_meteredconnections from egwtr_stg_connection where connectiontype='METERED';
	select count(*) into v_totaldcbs from egwtr_stg_connection_dcb;

	raise notice '**********************************************';
	raise notice 'Total no.of connections : % ', v_totalconnections;
	raise notice 'Total no.of metered connections : % ', v_meteredconnections;
	raise notice 'Total no.of non metered connections : % ', v_nonmeteredconnections;
	...
	v_validate=false;
	--Common
	select count(*) into v_noparentboundaries from eg_boundary where parent is null and boundarytype in (select id from eg_boundary_type where name in ('Block', 'Locality'));
	select count(*) into v_billingtype from egwtr_stg_connection where billingtype not in ('STANDARD','CUSTOM');
	...
	select count(dcb.oldconsumernumber) into v_wrongdcbcount from egwtr_stg_connection_dcb dcb where (coalesce(chargeamount, 0)-coalesce(chargecollection, 0)) + (coalesce(penaltyamount, 0)-coalesce(penaltycollection, 0)) + (coalesce(interestamount, 0)-coalesce(interestcollection, 0))>0 and coalesce(advancecollection, 0)>0;
	v_duplicatconnection := coalesce(v_duplicatconnection, 0);
	v_wrongdcbcount := coalesce(v_wrongdcbcount, 0);

	if (v_noparentboundaries > 0) then
		raise notice 'There are % boundary(s) of type Locality and Block where there is no parent', v_noparentboundaries;
	end if;
	if (v_billingtype > 0) then
		raise notice 'There are % connection(s) where billing type is invalid', v_billingtype;
	end if;
	...
	if (v_wrongdcbcount > 0) then
		raise notice 'There are % connection(s) with advance collection and billig cycle dues', v_wrongdcbcount;
	end if;

	if (v_noparentboundaries = 0 and v_billingtype = 0 and v_noofownernull = 0 and v_nooflocalitynull = 0 and v_noofpincodenull = 0 and v_noofoldconsumernumbernull = 0 and v_noofconnectiondatenull=0 and v_noofconnectiontypenull = 0 and v_noofwatersourcenull = 0 and v_noofpropertytypenull = 0 and v_noofcategorynull = 0 and v_noofusagetypenull = 0 and v_noofpipesizenull = 0  and v_duplicatconnection = 0 and v_wrongdcbcount = 0) then
	...
	select count(*) into v_invalidbillingcycle from egwtr_stg_connection_dcb where billingcycle not in (select description from eg_installment_master where id_module=(select id from eg_module where name='Water Tax Management'));
	...
	if (v_invalidbillingcycle > 0) then
		raise notice 'There are % connection(s) with invalid Billing Cycle', v_invalidbillingcycle;
	end if;
	if (v_invalidlocality = 0 and v_invalidconnectiontype = 0 and v_invalidwatersource = 0 and v_invalidpropertytype = 0 and v_invalidcategory = 0 and v_invalidusagetype = 0 and v_invalidpipesize = 0 and v_invalidbillingcycle=0) then
	...
	--DCB data reference
	v_validate=false;
	select count(*) into v_invalidconnectionno from egwtr_stg_connection_dcb where oldconsumernumber not in (select oldconsumernumber from egwtr_stg_connection);
	select count(*) into v_duplicatebillingcycle from egwtr_stg_connection_dcb group by oldconsumernumber, billingcycle having count(*) > 1;
	v_duplicatebillingcycle := coalesce(v_duplicatebillingcycle, 0);	
	if (v_invalidconnectionno > 0) then
		raise notice 'There are % connection(s) without connection data but has dcb data', v_invalidconnectionno;
	end if;
	if (v_duplicatebillingcycle > 0) then
		raise notice 'There are % connection(s) with duplicate DCB for same billig cycles', v_duplicatebillingcycle;
	end if;
	if (v_invalidconnectionno = 0 and v_duplicatebillingcycle = 0) then
	...
	select count(*) into v_noofmeterstatusnull from egwtr_stg_connection where connectiontype='METERED' and (meterstatus is null or length(trim(meterstatus))=0);
	select count(*) into v_noofinvalidmeterstatus from egwtr_stg_connection where connectiontype='METERED' and meterstatus is not null and length(trim(meterstatus))>0 and meterstatus not in ('Working', 'Locked', 'Breakdown', 'No_Meter', 'Reset', 'Replacement');

	if (v_noofmeterserialnonull > 0) then
		raise notice 'There are % metered connection(s) where there is no serial number', v_noofmeterserialnonull;
	end if;
	if (v_nooflatestreadingnull > 0) then
		raise notice 'There are % metered connection(s) where there is no latest reading', v_nooflatestreadingnull;
	end if;
	if (v_nooflatestreadingdatenull > 0) then
		raise notice 'There are % metered connection(s) where there is no latest reading date', v_nooflatestreadingdatenull;
	end if;
	if (v_noofavglastthreereadingnull > 0) then
		raise notice 'There are % metered connection(s) where there is no Average Meter Reading (Last 3 Bills)', v_noofavglastthreereadingnull;
	end if;
	if (v_noofmeterstatusnull > 0) then
		raise notice 'There are % metered connection(s) where there is no meter status', v_noofmeterstatusnull;
	end if;
	if (v_noofinvalidmeterstatus > 0) then
		raise notice 'There are % metered connection(s) where there is invalid meter status', v_noofinvalidmeterstatus;
	end if;

	if (v_noofmeterserialnonull = 0 and v_nooflatestreadingnull = 0 and v_nooflatestreadingdatenull = 0 and v_noofavglastthreereadingnull = 0 and v_noofmeterstatusnull=0 and v_noofinvalidmeterstatus = 0) then
		raise notice '*****Metered validations passed*****';
		v_validate=true;
	else
		raise notice '***Failed***Metered validations Failed***Failed***';
		return false;
	end if;
	return v_validate;
END;

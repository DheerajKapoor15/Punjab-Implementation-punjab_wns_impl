	  	...
	  	loop
			begin
				raise notice 'Started bill generation for consumercode: %',rec.consumercode;
	    			...
	    			v_temp := wtms_bill_genaration_for_consumer_code(rec.consumercode,rec.connid,rec.conndetid, in_curr_inst_id, in_prev_inst_id,curr_installment_description,rec.locationid,getbilltype_for_bill_generation(rec.consumercode));
...
v_penalty_glcode, v_function_code, v_penalty_amount, 0, v_billDesc, v_curr_inst_id, 1,'PENALTY');
	end  if;

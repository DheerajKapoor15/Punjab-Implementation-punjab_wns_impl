...
BEGIN
		...
		loop 
			...
			for installments in (select distinct d.id_installment, inst.description, inst.end_date from egswtax_demand_connection cdemand, eg_demand d, eg_installment_master inst where cdemand.demand=d.id and d.id_installment=inst.id and cdemand.applicationdetail = v_applicationdetailid)
			loop
				begin
					...
					select demandid into v_sdemandid from egwtr_demandbill_view where demandid=v_demandid;
					if(v_sdemandid is null) then
						...
						v_projecteddemand := stms_rates_by_connectiondetails_id(v_applicationdetailid, v_installmentid, installments.end_date::date);

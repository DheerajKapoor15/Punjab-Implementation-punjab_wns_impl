CREATE OR REPLACE FUNCTION pbsc_cancelseweragedemand(
...
BEGIN 
	...
	--insert into egswtax_backend_cancelrequest (id, consumercode, billingcycle, type, status, requestedtime) values (v_seq_cancelreq, in_consumercode, in_billingcycle, 'DEMAND', 'RECEIVED', now());
	--select count(*) into temp from egswtax_backend_cancelrequest;
	--raise notice 'request logged % ', temp;
	...
	if(v_inputdemandid is null)then
		--update egswtax_backend_cancelrequest set status='FAILED', error='The is no active demand available for given billing cycle' where id=v_seq_cancelreq;
	...
	if(v_previousdemandid is null)then
		--update egswtax_backend_cancelrequest set status='FAILED', error='The connection is either migrated or created with given billing cycle' where id=v_seq_cancelreq;

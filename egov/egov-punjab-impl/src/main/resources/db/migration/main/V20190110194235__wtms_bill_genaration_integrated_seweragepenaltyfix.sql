...
begin
	raise notice 'stms_penalty_and_interest_generate_for_consumercode in_wconsumercode, in_sconsumercode, in_curr_inst_id, in_prev_inst_id, in_bill_type % % % % %',in_wconsumercode, in_sconsumercode, in_curr_inst_id, in_prev_inst_id, in_bill_type;
	v_penalty := 0;
	...
	if(v_billdate is null)then
		...
		select issue_date into v_billdate from eg_bill where consumer_id=in_wconsumercode and module_id = (select id from eg_module where name='Water Tax Management') and id_bill_type != (select id from eg_bill_type where code='AUTO') and is_cancelled = 'N' and id_demand=v_previouswaterdemandid;
	end if;

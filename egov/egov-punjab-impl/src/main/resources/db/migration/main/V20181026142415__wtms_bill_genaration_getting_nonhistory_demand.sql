...
begin
	...
	if(v_currdemand is not null) then
		...
		select id into v_demandbill from eg_bill where consumer_id=in_consumercode and module_id = v_moduleid and id_bill_type not in (select id from eg_bill_type where code='AUTO') and is_cancelled='N' and id_demand = v_currdemand ;

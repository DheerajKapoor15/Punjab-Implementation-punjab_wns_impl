...
BEGIN
	...
	--exempted case validation starts
		if(v_demandAmountfromSWRate is not null and v_demandAmountfromSWRate = 0) then
			if (batch_id is null or batch_id = 0) then
				raise exception 'Consumer number is exempted from tax: %',in_consumercode ;				else
				v_status := false;
				insert into egswtax_batchjobdetailslog(id, jobdetails, consumercode, status, reason, createddate, lastmodifieddate, createdby, lastmodifiedby) values(nextval('seq_egstax_batchjobdetailslog'), batch_id, in_consumercode, 'Success', 'Consumer number is exempted from tax', now(), now(), (select id from eg_user where username='egovernments'), (select id from eg_user where username='egovernments'));
			end if;

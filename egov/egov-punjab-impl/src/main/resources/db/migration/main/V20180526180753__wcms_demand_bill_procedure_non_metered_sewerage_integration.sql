CREATE OR REPLACE FUNCTION wtms_bill_genaration_for_non_metered_connections(batch_name varchar,boundary_type varchar,curr_installment_description varchar, bill_type varchar)
RETURNS void AS
$BODY$
DECLARE
props record;
...
raise notice 'v_curr_inst_id,v_curr_inst_id_temp % %',v_curr_inst_id,v_curr_inst_id_temp;
	...
	if(v_curr_inst_id != v_curr_inst_id_temp) then
		...
		loop 
			...
			v_temp := wtms_bill_genaration_for_location(boundary_type,props.id,v_curr_inst_id,v_prev_inst_id,curr_installment_description,bill_type);
...
create or replace function wtms_bill_genaration_for_location(boundary_type varchar,in_location_id bigint,in_curr_inst_id bigint,in_prev_inst_id bigint,curr_installment_description varchar, in_bill_type varchar)
returns numeric as 
$BODY$
declare
rec record;
...
create or replace function wtms_bill_genaration_for_consumer_code(in_consumercode varchar(16), in_conn_id bigint,in_conn_det_id bigint,in_curr_inst_id bigint,in_prev_inst_id bigint,curr_installment_description varchar,in_location_id bigint, in_bill_type varchar)
returns numeric  as 
$BODY$
declare
...
begin
	...
	if(v_currdemand is not null) then
		...
		if(v_demandbill is null) then
			...
			insert into eg_bill(id, id_demand, citizen_name, citizen_address, bill_no, id_bill_type, issue_date, last_date, module_id, user_id, create_date, modified_date, is_history, is_cancelled, fundcode, functionary_code,fundsource_code, department_code, coll_modes_not_allowed, boundary_num, boundary_type,total_amount, total_collected_amount, service_code, part_payment_allowed, override_accountheads_allowed,description, min_amt_payable, consumer_id, dspl_message, callback_for_apportion,penalty) values (v_billpk, v_currdemand, v_ownername,v_houseno, v_billno, v_idbillype, now(), now(), v_moduleid, v_iduser, now(), now(), 'N', 'N', '01', 1, '01', 'REV', null, in_location_id, 'Ward', v_totalamount, v_totalamountcollected, 'WT', 'Y', 'N', v_description, 0, in_consumercode, 'Water Charge Collection', 'N',v_penalty);
...
begin
	...
	if(v_billdate is not null) then
		...
		if(v_penalty_date < now() and v_previousChargesBal>0) then
			...
			select id into v_penalty_demand_reason_id from eg_demand_reason where id_demand_reason_master in(select id from eg_demand_reason_master where code='PENALTY') and id_installment=in_prev_inst_id;
		...
		if(v_interest_date < now() and v_previousChargesBal>0) then
			...
			select id into v_interest_demand_reason_id from eg_demand_reason where id_demand_reason_master in(select id from eg_demand_reason_master where code='INTEREST') and id_installment=in_prev_inst_id;
...
v_installmentDesc character varying(256);
v_billDesc character varying(256);
v_st_charges_demandreason bigint;
v_penalty_demandreason bigint;
v_interest_demandreason bigint;
v_module_id bigint;
v_arrear_amount double precision;
...
select id into v_module_id from eg_module where name='Sewerage Tax Management';
	...
	select id into v_st_charges_demandreason from eg_demand_reason where id_demand_reason_master in (select id from eg_demand_reason_master where code='SEWERAGETAX' and module = v_module_id) and id_installment=v_curr_inst_id;

	select id into v_penalty_demandreason from eg_demand_reason where id_demand_reason_master in (select id from eg_demand_reason_master where code='PENALTY' and module = v_module_id) and id_installment=v_curr_inst_id;

	select id into v_interest_demandreason from eg_demand_reason where id_demand_reason_master in (select id from eg_demand_reason_master where code='INTEREST' and module = v_module_id) and id_installment=v_curr_inst_id;
	...
	insert into eg_bill_details(id,id_demand_reason,create_date,modified_date,id_bill,collected_amount,order_no,glcode,function_code, cr_amount ,dr_amount,description ,id_installment ,  additional_flag,purpose) select nextval('seq_eg_bill_details'),  v_st_charges_demandreason, now(),now(), v_billpk,0, 1,'1806006','50515100',amount, 0,v_billDesc,v_curr_inst_id,1,'CURRENT_AMOUNT' from eg_demand_details dd where dd.id_demand=v_currdemand and dd.amount>0 and dd.id_demand_reason=v_st_charges_demandreason ;
...
-- arrear amount
	...
	if(v_arrear_amount is not null and v_arrear_amount > 0) then 
		insert into eg_bill_details(id, id_demand_reason, create_date, modified_date,id_bill, collected_amount, order_no, glcode, function_code, cr_amount, dr_amount, description, id_installment, additional_flag,purpose) values(nextval('seq_eg_bill_details'), v_st_charges_demandreason, now(), now(), v_billpk, 0, 2, '1806006', '50515100',v_arrear_amount, 0, v_billDesc, v_curr_inst_id, 1,'ARREAR_AMOUNT');
	end  if;


-- penalty amount

	select sum(dd.amount - dd.amt_collected) into v_penalty_amount from eg_demand_details dd where dd.id_demand_reason= v_penalty_demandreason and dd.id_demand=v_currdemand ;

	if(v_penalty_amount is not null and v_penalty_amount > 0) then 
		insert into eg_bill_details(id, id_demand_reason, create_date, modified_date,id_bill, collected_amount, order_no, glcode, function_code, cr_amount, dr_amount, description, id_installment, additional_flag,purpose) values(nextval('seq_eg_bill_details'), v_penalty_demandreason, now(), now(), v_billpk, 0, 2, 
'1806006', '50515100', v_penalty_amount, 0, v_billDesc, v_curr_inst_id, 1,'PENALTY');
	end  if;


-- interest amount

	select sum(dd.amount - dd.amt_collected) into v_interest_amount from eg_demand_details dd where dd.id_demand_reason= v_interest_demandreason and dd.id_demand=v_currdemand ;

	if(v_interest_amount is not null and v_interest_amount > 0) then 
		insert into eg_bill_details(id, id_demand_reason, create_date, modified_date,id_bill, collected_amount, order_no, glcode, function_code, cr_amount, dr_amount, description, id_installment, additional_flag,purpose) values(nextval('seq_eg_bill_details'), v_interest_demandreason, now(), now(), v_billpk, 0, 2,'1806006', '50515100', v_interest_amount, 0, v_billDesc, v_curr_inst_id, 1,'INTEREST');
	end  if;

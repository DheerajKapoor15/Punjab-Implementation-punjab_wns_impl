DROP FUNCTION IF EXISTS wtms_demand_genaration(bigint,varchar,varchar,varchar,bigint);
...
v_citycode varchar;
	...
	select code into v_connectiontypename from egwtr_connection_type where id=connection_type;
		select usagetype into v_usagetype from egwtr_connectiondetails where connection in (select id from egwtr_connection where consumercode = in_consumer_code) and connectionstatus='ACTIVE';
	...
	select extract(epoch from executiondate) into v_executiondate from egwtr_connectiondetails where connection in (select id from egwtr_connection where consumercode = in_consumer_code) and connectionstatus='ACTIVE' ;
		...
		--exempted case validation ends
			...
			if(v_demandAmountfromWtRate is not null and v_demandAmountfromWtRate > 0) then
				...
				if (v_olddemandid IS NOT NULL) then 
					...
					insert into eg_demand_details(id, id_demand, id_demand_reason, id_status, file_reference_no, remarks, amount, modified_date, create_date, amt_collected, amt_rebate) select nextval('seq_eg_demand_details'), v_newdemandid, id_demand_reason, id_status, file_reference_no, remarks, amount, modified_date, create_date, amt_collected, amt_rebate from eg_demand_details where id_demand = v_olddemandid and id_demand_reason not in (select id from eg_demand_reason where id_demand_reason_master in (select id from eg_demand_reason_master where code in('WTADVANCE','WTADJUSTMENT')));
...
BEGIN
	...
	if(v_billingtype is not null and v_billingtype = 'CUSTOM') then 
		...
		if(v_connectiontype is not null and v_connectiontype = v_connectiontypeid) then
			...
			if(v_ratetype is not null and v_ratetype = 'FIXED') then
				select fixedCharges into v_rate from egwtr_watercharge_rates where usagetype = v_usagetype and ratetype = 'FIXED' and effectiveFromDate<=in_curr_instl_end_date and effectiveToDate>=in_curr_instl_end_date ;
				raise notice 'Got data from rates master by fixed is:%',v_rate;
			end if;

			if(v_ratetype is not null and v_ratetype = 'TAP_BASED') then
			
				for rates in (select fromNoOfTaps,toNoOfTaps,rateAmount,fixedCharges from egwtr_watercharge_rates where usagetype = v_usagetype and effectiveFromDate<=in_curr_instl_end_date and effectiveToDate>=in_curr_instl_end_date)
				loop
					begin
						if(rates.fromNoOfTaps is not null and rates.fromNoOfTaps <= v_numberoftaps and rates.toNoOfTaps is not null and rates.toNoOfTaps <= v_numberoftaps and rates.fixedCharges is null) then
						...
						if(rates.fromNoOfTaps is not null and rates.fromNoOfTaps <= v_numberoftaps and rates.toNoOfTaps is null and rates.fixedCharges is null) then
							v_diff := ( v_numberoftaps - rates.fromNoOfTaps) + 1;
							v_temprate := v_temprate + (v_diff * rates.rateAmount);
						end if;
						if(rates.fromNoOfTaps is null and rates.toNoOfTaps is null and rates.fixedCharges is not null) then
							v_temprate :=rates.fixedCharges;
						end if;
					end;
				end loop;
				v_rate := v_temprate;
...
v_plotsize numeric;
v_usagetypefixedmohali numeric;
v_usagetypedomestc numeric;
v_citycode character varying(4);
v_temprate numeric default 0;
rates record;
BEGIN
	
	select id into v_installment_id from eg_installment_master where id = in_curr_inst_id ;
        select code into v_citycode from eg_city where active = true;
	select id into v_connectiontypeid from egwtr_connection_type where code = 'METERED';
	select plotSize into v_plotsize from egwtr_connectiondetails where id = conn_detail_id;
	select id into v_usagetypefixedmohali from egwtr_usage_type where name = 'DOMESTIC_FIXED';
   	select id into v_usagetypedomestc from egwtr_usage_type where name = 'DOMESTIC';
	...
	select c.consumercode, cd.billingType, cd.connectionType, cd.usageType, cd.averageMeterReading into v_consumercode, v_billingtype, v_connectiontype, v_usagetype, v_averageMeterReading from egwtr_connectiondetails cd,egwtr_connection c where cd.connection = c.id and cd.id = conn_detail_id;
	     if ( (v_citycode = '2005' or v_citycode = '1104' or v_citycode = '1702' or v_citycode='1014' or v_citycode='1407') and v_usagetype=v_usagetypedomestc and v_plotsize is not null and v_plotsize >= 125 and v_installment_id >=128 ) then
     		v_rateAmount:=150;		
      else
	       if ((v_citycode = '2005' or v_citycode = '1104' or v_citycode = '1702' or v_citycode='1014' or v_citycode='1407') and v_usagetype=v_usagetypedomestc and v_plotsize is not null and v_plotsize < 125 and v_installment_id >=128 ) then
     		v_rateAmount:=0;		
      else
     if (v_citycode = '1508' and v_usagetype=v_usagetypefixedmohali and v_plotsize is not null and v_plotsize >= 125 and v_installment_id >=128 ) then
     		v_rateAmount:=150;		
      else
	       if (v_citycode = '1508' and v_usagetype=v_usagetypefixedmohali and v_plotsize is not null and v_plotsize < 125 and v_installment_id >=128 ) then
     		v_rateAmount:=0;				
    ...
    v_diff := ( rates.toVolume - rates.fromVolume);
    v_temprate := v_temprate + (v_diff * rates.rateAmount);
end if;
                       
if(rates.fromVolume is not null and rates.fromVolume <= v_unitsconsumed and rates.toVolume is not null and rates.toVolume > v_unitsconsumed) then
    v_diff := ( v_unitsconsumed - rates.fromVolume);
    v_temprate := v_temprate + (v_diff * rates.rateAmount);
end if;
...
CREATE OR REPLACE FUNCTION wtms_demand_generation_titletransfer_for_consumer_code(application_type_code varchar,connection_type bigint,in_consumer_code varchar,in_idconndetail bigint, in_curr_inst_id bigint, in_prev_inst_id bigint, batch_id bigint,applicationdate date)
RETURNS numeric AS
$BODY$
DECLARE
v_olddemandid bigint;
v_newdemandid bigint;
v_currdemandid bigint;
v_exceution_date_instllid bigint;
v_temp integer;
v_demandAmountfromWtRate double precision;
v_curr_instl_end_date date;
v_prev_bill_id bigint;
...
v_days_btw_conn_date double precision;
v_days_btw_quarter double precision;
v_applicationdate double precision;
v_curr_inst_fromdate double precision;
v_executiondate date;
v_executiondatetime timestamp without time zone;
v_isexempted boolean default false;



BEGIN
 
	raise notice 'wtms_demand_generation_titletransfer_for_consumer_code,application_type_code,connection_type ,in_consumer_code, in_idconndetail, in_curr_inst_id, in_prev_inst_id ,batch_id,applicationdate % % % % % % % %',application_type_code,connection_type, in_consumer_code, in_idconndetail, in_curr_inst_id, in_prev_inst_id,batch_id,applicationdate;
	
	select id into v_connectiontype from egwtr_connection_type where code = 'NON_METERED';

	--validation for application date starts

v_applicationdate:=extract(epoch from applicationdate);


	--validation for execution date validation starts
	select executiondate into v_executiondate from egwtr_connectiondetails where connection in (select id from egwtr_connection where consumercode = in_consumer_code)  and connectionstatus='ACTIVE';
			raise notice 'Execution date is %',v_executiondate;
SELECT v_executiondate::timestamp without time zone into v_executiondatetime;

	raise notice 'updated Execution date is %',v_executiondate;
select id into v_exceution_date_instllid  from eg_installment_master where start_date <= v_executiondatetime and end_date >=v_executiondatetime and id_module=(select id from eg_module where name ='Water Tax Management');
			raise notice 'Execution date installment id  is %',v_exceution_date_instllid;

	select extract(epoch from start_date) into v_curr_inst_fromdate from eg_installment_master where id = in_curr_inst_id;

	if (v_applicationdate is null) then
		if (batch_id is null or batch_id = 0) then
			raise exception 'Water Application for Title Transfer not received for : %',in_consumer_code ;
		else
			v_status := false;
			insert into egwtr_batchjobdetailslog(id, jobdetails, consumercode, status, reason, createddate, lastmodifieddate, createdby, lastmodifiedby) values(nextval('seq_egwtr_batchjobdetailslog'), batch_id, in_consumer_code, 'Failure', 'Request Water for Title Transfer not received', now(), now(), (select id from eg_user where username='egovernments'), (select id from eg_user where username='egovernments'));
		end if;			
	end if;


	if ( (v_applicationdate-v_curr_inst_fromdate) < 0) then
		if (batch_id is null or batch_id = 0) then
			raise exception 'For Water Connectuon,Billing cycle period is after the execution date for Water consumer : %',in_consumer_code ;
		else
			v_status := false;
			insert into egwtr_batchjobdetailslog(id, jobdetails, consumercode, status, reason, createddate, lastmodifieddate, createdby, lastmodifiedby) values(nextval('seq_egwtr_batchjobdetailslog'), batch_id, in_consumer_code, 'Failure', 'For Water Connection,Billing cycle period is after the application Date', now(), now(), (select id from eg_user where username='egovernments'), (select id from eg_user where username='egovernments'));
		end if;			
	end if;
	...
	--validation for sequence of billing cycles starts
	select inst.id into v_connlatestinstid from eg_demand d, egwtr_demand_connection dc, eg_installment_master inst, eg_demand_details dd, eg_demand_reason_master dm, eg_demand_reason dr where dc.connectiondetails=in_idconndetail and dc.demand=d.id and inst.id=d.id_installment and d.id=dd.id_demand and dd.id_demand_reason=dr.id and dr.id_demand_reason_master=dm.id and dm.code='WTAXCHARGES' and d.is_history='N' order by inst.start_date desc limit 1;
raise notice 'wtms_demand_generation_titletransfer_for_consumer_code v_connlatestinstid %',v_connlatestinstid;
 select isexempted into v_isexempted from egwtr_connectiondetails where connection in (select id from egwtr_connection where consumercode = in_consumer_code)  and connectionstatus='ACTIVE';

if(v_connlatestinstid is null and v_exceution_date_instllid != in_curr_inst_id and v_isexempted != true) then
	if (batch_id is null or batch_id = 0) then
			raise exception 'Please generate previous quarter water demand for : %',in_consumer_code ;
		else
			v_status := false;
			insert into egwtr_batchjobdetailslog(id, jobdetails, consumercode, status, reason, createddate, lastmodifieddate, createdby, lastmodifiedby) values(nextval('seq_egwtr_batchjobdetailslog'), batch_id, in_consumer_code, 'Failure', 'Water Demand is already generated for given billing cycle', now(), now(), (select id from eg_user where username='egovernments'), (select id from eg_user where username='egovernments'));
		end if;
end if;

	if(v_connlatestinstid is not null and v_connlatestinstid = in_curr_inst_id) then

		if (batch_id is null or batch_id = 0) then
			raise exception 'Water Demand is already generated for given billing cycle: %',in_consumer_code ;
		else
			v_status := false;
			insert into egwtr_batchjobdetailslog(id, jobdetails, consumercode, status, reason, createddate, lastmodifieddate, createdby, lastmodifiedby) values(nextval('seq_egwtr_batchjobdetailslog'), batch_id, in_consumer_code, 'Failure', 'Water Demand is already generated for given billing cycle', now(), now(), (select id from eg_user where username='egovernments'), (select id from eg_user where username='egovernments'));
		end if;
	...
	if(v_connlatestinstid is not null and v_exceution_date_instllid != in_curr_inst_id and v_isexempted != true) then
		select abs(extract(epoch from (currinst.start_date-previnst.end_date))) into v_instdiffinsecs from eg_installment_master previnst, eg_installment_master currinst where previnst.id=v_connlatestinstid and currinst.id=in_curr_inst_id;
	else 
		v_instdiffinsecs := 1;
	end if;
	if(v_instdiffinsecs>1)then
		raise notice 'For Water Connection,the difference between end date of previous billing cycle and start date of current billing cycle provide is (in secs) %',v_instdiffinsecs;
		if (batch_id is null or batch_id = 0) then
			raise exception 'For Water Connection,Demand generation is not allowed as previous bill/demand are not generated';
		else
			v_status := false;
			insert into egwtr_batchjobdetailslog(id, jobdetails, consumercode, status, reason, createddate, lastmodifieddate, createdby, lastmodifiedby) values(nextval('seq_egwtr_batchjobdetailslog'), batch_id, in_consumer_code, 'Failure', 'For Water Connection,Demand generation is not allowed as previous bill/demand are not generated', now(), now(), (select id from eg_user where username='egovernments'), (select id from eg_user where username='egovernments'));
		end if;
	--validation for sequence of billing cycles ends
	else 
	select min(d.id) into v_firstdemandid from eg_demand d, egwtr_demand_connection dc where dc.connectiondetails=in_idconndetail and dc.demand=d.id;
	...
	--raise notice 'v_is_oldfirstdemand----- %', v_is_oldfirstdemand;
	raise notice 'wtms_demand_generation_titletransfer_for_consumer_code,  v_olddemandid, v_currdemandid, v_prev_bill_id % % %',v_olddemandid, v_currdemandid, v_prev_bill_id;

	if(v_currdemandid is null) then
		if((v_olddemandid is not null and v_prev_bill_id is not null) or (v_olddemandid is null)) then
			raise notice 'wtms_demand_generation_titletransfer_for_consumer_code, v_olddemandid is not null';
			select end_date into v_curr_instl_end_date from eg_installment_master where id = in_curr_inst_id;
			raise notice 'wtms_demand_generation_titletransfer_for_consumer_code, v_curr_instl_end_date % ', v_curr_instl_end_date;
			
			--prorating logic starts
			
			if(connection_type is not null and connection_type = v_connectiontype) then
			
				v_conn_date:= applicationdate;

				select start_date into v_curr_instl_start_date from eg_installment_master where id = in_curr_inst_id;
		
				if (v_curr_instl_start_date <= v_conn_date and v_curr_instl_end_date >= v_conn_date and v_curr_instl_start_date >= v_executiondate) then
					select wtms_rates_by_connectiondetails_id_for_non_metered(in_idconndetail,v_curr_instl_end_date) into v_demandAmountfromWtRate;
					v_days_btw_conn_date := abs(v_conn_date - v_curr_instl_start_date) ;
					v_days_btw_quarter := abs(v_curr_instl_end_date - v_curr_instl_start_date) + 1;
					v_demandAmountfromWtRate := round(v_days_btw_conn_date * (v_demandAmountfromWtRate / v_days_btw_quarter));
				
				else if (v_curr_instl_start_date <= v_executiondate and v_curr_instl_end_date >= v_executiondate and v_curr_instl_start_date <= v_conn_date and v_curr_instl_end_date >= v_conn_date) then
					select wtms_rates_by_connectiondetails_id_for_non_metered(in_idconndetail,v_curr_instl_end_date) into v_demandAmountfromWtRate;
					v_days_btw_conn_date := abs(v_conn_date - v_executiondate);
					v_days_btw_quarter := abs(v_curr_instl_end_date - v_curr_instl_start_date) + 1;
					v_demandAmountfromWtRate := round(v_days_btw_conn_date * (v_demandAmountfromWtRate / v_days_btw_quarter));
		...
		--exempted case validation starts
			if(v_demandAmountfromWtRate is not null and v_demandAmountfromWtRate = 0) then
				if (batch_id is null or batch_id = 0) then
					raise exception 'Water Connection with consumer number: % is exempted. Hence the water charges is zero.',in_consumer_code ;					
				else
					v_status := false;
					insert into egwtr_batchjobdetailslog(id, jobdetails, consumercode, status, reason, createddate, lastmodifieddate, createdby, lastmodifiedby) values(nextval('seq_egwtr_batchjobdetailslog'), batch_id, in_consumer_code, 'Success', 'Water Connection with consumer number: ' || in_consumer_code || ' is exempted. Hence the water charges is zero.', now(), now(), (select id from eg_user where username='egovernments'), (select id from eg_user where username='egovernments'));
				end if;
		...
		--exempted case validation ends
			...
			if(v_demandAmountfromWtRate is not null and v_demandAmountfromWtRate > 0) then
				...
				if (v_olddemandid IS NOT NULL) then 
					...
					insert into eg_demand (id, id_installment, base_demand, is_history, create_date, modified_date, amt_collected, status, min_amt_payable, amt_rebate) select v_newdemandid, in_curr_inst_id, base_demand, is_history, now(), now(),amt_collected, status, null, 0 from eg_demand where id = v_olddemandid;
					--copying old demand_demand_details to new eg_demand.
					raise notice 'wtms_demand_generation_titletransfer_for_consumer_code, copying old demand_demand_details to new eg_demand.';
					insert into eg_demand_details(id, id_demand, id_demand_reason, id_status, file_reference_no, remarks, amount, modified_date, create_date, amt_collected, amt_rebate) select nextval('seq_eg_demand_details'), v_newdemandid, id_demand_reason, id_status, file_reference_no, remarks, amount, modified_date, create_date, amt_collected, amt_rebate from eg_demand_details where id_demand = v_olddemandid and id_demand_reason not in (select id from eg_demand_reason where id_demand_reason_master in (select id from eg_demand_reason_master where code in('WTADVANCE','WTADJUSTMENT'))); 
					raise notice 'copied old demand details to new eg_demand for consumer code: %',in_consumer_code;
			...
			if (batch_id is null or batch_id = 0) then
				raise exception 'Water Bill is not generated for previous billing cycle: %',in_consumer_code ;			
			else
				v_status := false;
				insert into egwtr_batchjobdetailslog(id, jobdetails, consumercode, status, reason, createddate, lastmodifieddate, createdby, lastmodifiedby) values(nextval('seq_egwtr_batchjobdetailslog'), batch_id, in_consumer_code, 'Failure', 'Bill is not generated for previous billing cycle', now(), now(), (select id from eg_user where username='egovernments'), (select id from eg_user where username='egovernments'));
			end if;
	...
	--success state
	if (batch_id is not null and batch_id <> 0 and v_status) then
		insert into egwtr_batchjobdetailslog(id, jobdetails, consumercode, status, reason, createddate, lastmodifieddate, createdby, lastmodifiedby) values(nextval('seq_egwtr_batchjobdetailslog'), batch_id, in_consumer_code, 'Success', 'Water Demand is generated successfully for consumer number: ' || in_consumer_code, now(), now(), (select id from eg_user where username='egovernments'), (select id from eg_user where username='egovernments'));
	end if;
	return v_temp;
END;
$BODY$ LANGUAGE plpgsql;

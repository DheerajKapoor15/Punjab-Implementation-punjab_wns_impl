...
CREATE OR REPLACE FUNCTION pbwc_migrateLocalityConnections(localityCode IN bigint)
RETURNS void AS
$BODY$
DECLARE
connections egwtr_stg_connection%ROWTYPE;
...
BEGIN
	begin
		...
		select connection into v_idconnection from egwtr_connectiondetails where connection=v_idconnection;
		...
		update egwtr_stg_connection set ismigrated='Y' where oldconsumernumber=oldConnectionNo;
...
BEGIN 
	...
	select houseno, 'WTCONNECTION_ADDRESS', street, locality, pincode, v_addresspk, userid from egwtr_stg_connection where oldconsumernumber = oldConnectionNo;
...
INSERT INTO egwtr_connection(id, oldconsumernumber, consumercode, propertyidentifier, address, sewerageidentifier, zone, ward, block, locality, createdby, createddate, lastmodifieddate, lastmodifiedby, version)
	select v_connectionpk, oldconsumernumber, v_connectionno, propertyidentifier, addressid, sewerageconnectionnumber, null, null, null, (select id from eg_boundary where code=locality), 1, connectiondate, connectiondate, 1, 0 from egwtr_stg_connection where oldconsumernumber = oldConnectionNo;
	--raise notice 'createConnection connection created';
INSERT INTO  egwtr_connectiondetails (id, connection, applicationdate, applicationtype, billingtype, billamount, plotsize, nooftaps, ledgernumber, billgroupnumber, connectiontype, watersource, propertytype, category, usagetype, pipesize, donationcharges, connectionstatus, legacy, statusid, createdby, createddate, lastmodifieddate, lastmodifiedby, version)
	select v_connectiondetpk, v_connectionpk, connectiondate, (select id from egwtr_application_type where code='NEWCONNECTION'), billingtype, custombillamount, plotsize, nooftaps, ledgerid, billgroupid, connectiontype, (select id from egwtr_water_source where code=watersource), (select id from egwtr_property_type where name=propertytype), (select id from egwtr_category where name=category), (select id from egwtr_usage_type where name=usagetype), (select id from egwtr_pipesize where code=pipesize), securitydeposit, coalesce(status,'ACTIVE'), false, (select id from egw_status where moduletype='WATERTAXAPPLICATION' and code='CREATED'), 1, connectiondate, connectiondate, 1, 0 from egwtr_stg_connection where oldconsumernumber = oldConnectionNo;

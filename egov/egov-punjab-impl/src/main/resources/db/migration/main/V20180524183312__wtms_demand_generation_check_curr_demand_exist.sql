...
BEGIN
	...
	select id into v_tempdemandid from eg_demand where id in (select demand from egwtr_demand_connection where connectiondetails=in_idconndetail) and id_installment = in_curr_inst_id;
	...
	if(v_tempdemandid is null ) then

		if((v_olddemandid is not null and v_prev_bill_id is not null)) then
		...
		raise notice 'Demand is already exist for given installment: %',in_consumer_code ;
	end if;

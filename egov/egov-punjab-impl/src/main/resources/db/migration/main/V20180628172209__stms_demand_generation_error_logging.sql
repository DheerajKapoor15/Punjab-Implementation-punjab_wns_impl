DROP FUNCTION IF EXISTS stms_demand_generation_for_consumer_code(in_consumercode varchar,in_idconndetail bigint, in_curr_inst_id bigint, in_prev_inst_id bigint);

DROP FUNCTION IF EXISTS stms_demand_genaration_for_boundary(boundary_type varchar, in_location_id bigint, in_curr_inst_id bigint, in_prev_inst_id bigint);

DROP FUNCTION IF EXISTS stms_demand_genaration(batch_name varchar,boundary_type varchar,curr_installment_description varchar);
...
BEGIN
	...
	loop 
		...
		if (batch_id is null) then
			...
			insert into egswtax_batchjobdetailslog(id, jobdetails, consumercode, status, reason, createddate, lastmodifieddate, createdby, lastmodifiedby) values(nextval('seq_egstax_batchjobdetailslog'), batch_id, null, 'Failure', 'Error in demand generation for ulb ' || props.name || ' ' || SQLERRM || ' ' || SQLSTATE, now(), now(), (select id from eg_user where username='egovernments'), (select id from eg_user where username='egovernments'));
		end if;
		end;
	end loop;
...
BEGIN
	...
	if(boundary_type is not null and boundary_type = 'Locality') then 
		...
		loop 
			...
			when others then
				if (batch_id is null) then
					raise notice 'Error in demand generation for consumercode: % % %',props.shsc_number,SQLERRM, SQLSTATE;
				else
					insert into egswtax_batchjobdetailslog(id, jobdetails, consumercode, status, reason, createddate, lastmodifieddate, createdby, lastmodifiedby) values(nextval('seq_egstax_batchjobdetailslog'), batch_id, null, 'Failure', 'Error in demand generation for consumercode ' || props.shsc_number || ' ' || SQLERRM || ' ' || SQLSTATE, now(), now(), (select id from eg_user where username='egovernments'), (select id from eg_user where username='egovernments'));
			end if;
			end;
		end loop;
	end if;
	...
	if(v_currdemandid is null) then
		if((v_olddemandid is not null and v_prev_bill_id is not null) or (v_olddemandid is null)) then
			...
			if(v_demandAmountfromSWRate is not null and v_demandAmountfromSWRate > 0) then
				...
				if (batch_id is null) then
					raise exception 'No Water Rates present for cousumer code: %',in_consumercode ;				else
					v_status := false;
					insert into egswtax_batchjobdetailslog(id, jobdetails, consumercode, status, reason, createddate, lastmodifieddate, createdby, lastmodifiedby) values(nextval('seq_egstax_batchjobdetailslog'), batch_id, in_consumercode, 'Failure', 'No Water Rates present for cousumer number: ' || in_consumercode, now(), now(), (select id from eg_user where username='egovernments'), (select id from eg_user where username='egovernments'));
				end if;
			end if;
			else
				if (batch_id is null) then

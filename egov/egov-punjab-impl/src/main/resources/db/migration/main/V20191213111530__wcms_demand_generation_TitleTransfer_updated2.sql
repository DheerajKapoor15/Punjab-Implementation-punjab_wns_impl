drop function IF EXISTS wtms_rates_by_connectiondetails_id_for_metered(bigint,bigint,date,bigint);
drop function IF EXISTS wtms_demand_generation_titletransfer_for_consumer_code(varchar,bigint,varchar,bigint, bigint, bigint, bigint,date);
drop function IF EXISTS wtms_demand_generation_for_consumer_code(bigint,varchar,bigint, bigint, bigint, bigint);
DROP FUNCTION IF EXISTS wtms_rates_by_connectiondetails_id_for_non_metered(bigint,date);
				...
				if (v_citycode = '2001') then
					...
					loop
						begin
							...
							if(rates.fromVolume is not null and rates.fromVolume>0 and rates.fromVolume <= v_averageMeterReading and rates.toVolume is not null and rates.toVolume <= v_averageMeterReading) then
  							    v_diff := ( rates.toVolume - rates.fromVolume) + 0.99;
							    v_temprate := v_temprate + (v_diff * rates.rateAmount);
							end if;
							...
							if(rates.fromVolume is not null and rates.fromVolume <= v_averageMeterReading and rates.toVolume is not null and rates.toVolume > v_averageMeterReading) then
 							     v_diff := ( v_averageMeterReading - rates.fromVolume) + 0.99;
   								 v_temprate := v_temprate + (v_diff * rates.rateAmount);
							end if;

					...
					loop
						begin
							...
							if(rates.fromVolume is not null and rates.fromVolume>0 and rates.fromVolume <= v_averageMeterReading and rates.toVolume is not null and rates.toVolume <= v_averageMeterReading) then
  							    v_diff := ( rates.toVolume - rates.fromVolume) + 0.975;
							    v_temprate := v_temprate + (v_diff * rates.rateAmount);
							end if;

							if(rates.fromVolume is not null and rates.fromVolume = 0 and rates.fromVolume <= v_averageMeterReading and rates.toVolume is not null and rates.toVolume <= v_averageMeterReading) then
   								 v_diff := ( rates.toVolume - rates.fromVolume)-0.025;
   								 v_temprate := v_temprate + (v_diff * rates.rateAmount);
							end if;
							...
							if(rates.fromVolume is not null and rates.fromVolume <= v_averageMeterReading and rates.toVolume is not null and rates.toVolume > v_averageMeterReading) then
 							     v_diff := ( v_averageMeterReading - rates.fromVolume) + 0.975;
   								 v_temprate := v_temprate + (v_diff * rates.rateAmount);
							end if;
				...
				if (v_citycode = '2001') then
					...
					loop
						begin
							...
							if(rates.fromVolume is not null and rates.fromVolume>0 and rates.fromVolume <= v_unitsconsumed and rates.toVolume is not null and rates.toVolume <= v_unitsconsumed) then
  							    v_diff := ( rates.toVolume - rates.fromVolume) + 0.975;
							    v_temprate := v_temprate + (v_diff * rates.rateAmount);
							end if;
							...
							if(rates.fromVolume is not null and rates.fromVolume <= v_unitsconsumed and rates.toVolume is not null and rates.toVolume > v_unitsconsumed) then
 							     v_diff := ( v_unitsconsumed - rates.fromVolume) + 0.975;
   								 v_temprate := v_temprate + (v_diff * rates.rateAmount);
							end if;

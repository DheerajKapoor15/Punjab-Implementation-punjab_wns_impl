...
BEGIN
	...
	select c.consumercode, cd.billingType, cd.connectionType, cd.usageType, cd.averageMeterReading into v_consumercode, v_billingtype, v_connectiontype, v_usagetype, v_averageMeterReading from egwtr_connectiondetails cd,egwtr_connection c where cd.connection = c.id and cd.id = conn_detail_id;
	     if (v_citycode = '2005' and v_usagetype=v_usagetypedomestc and v_plotsize is not null and v_plotsize >= 125 ) then
     		v_rateAmount:=150;		
      else
	       if (v_citycode = '2005' and v_usagetype=v_usagetypedomestc and v_plotsize is not null and v_plotsize < 125 ) then
     		v_rateAmount:=0;		
      else
     if (v_citycode = '1508' and v_usagetype=v_usagetypefixedmohali and v_plotsize is not null and v_plotsize >= 125 ) then
     		v_rateAmount:=150;		
      else
	       if (v_citycode = '1508' and v_usagetype=v_usagetypefixedmohali and v_plotsize is not null and v_plotsize < 125 ) then
     		v_rateAmount:=0;		

...
BEGIN
		for connections in (select conn.shsc_number, conn.id as connid, cd.id as cdid, conn.zone, conn.locality, conn.block from egswtax_connection conn, egswtax_applicationdetails cd where conn.id=cd.connection and conn.status='ACTIVE')
		loop 
			begin
			...
			loop
				begin
					...
					insert into egwtr_demandbill_view (consumercode, servicetype, billingcycle, currentcharges, arrearcharges, penalty, interest, currentcollection, arrearcollection, penaltycollection, interestcollection, advancecollection, isbillgenerated, billgenerationdate, billduedate, billamount, billtype, demandlastupdatetime, billlastupdatedtime, demandid, zone, block, locality) values (connections.shsc_number, 'Sewerage Charges', v_billingcycle, v_currentcharges, v_arrearcharges, v_penaltycharges, v_interestcharges, v_currentchargecoll, v_arrearchargecoll, v_penaltycoll, v_interestcoll, v_advancecoll, v_isbillgenerated, v_billdate, v_billduedate, v_billamount, v_billtype, v_demandlastupdatedate, v_billlastupdatedate, v_demandid, v_zone, v_block, v_locality);
				EXCEPTION
				WHEN OTHERS THEN

...
drop FUNCTION wtms_demand_genaration_for_non_metered_connections_for_locality(boundary_type varchar,in_location_id bigint,in_curr_inst_id bigint,in_prev_inst_id bigint);
...
BEGIN
	...
	select max(inst.id) into v_connlatestinstid from eg_demand d, egwtr_demand_connection dc, eg_installment_master inst where dc.connectiondetails=in_idconndetail and dc.demand=d.id and inst.id=d.id_installment;
	...
	if(v_currdemandid is null) then
		if((v_olddemandid is not null and v_prev_bill_id is not null) or (v_olddemandid is null)) then
			raise notice 'wtms_demand_generation_for_consumer_code, v_olddemandid is null';

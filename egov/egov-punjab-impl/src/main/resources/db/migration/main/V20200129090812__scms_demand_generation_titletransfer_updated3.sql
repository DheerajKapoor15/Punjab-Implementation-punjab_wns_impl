drop function IF EXISTS stms_demand_generation_titletransfer_for_consumer_code(varchar,bigint,bigint,bigint,bigint,date);
drop FUNCTION IF EXISTS stms_demand_generation_for_consumer_code(varchar, bigint, bigint, bigint, bigint);
DROP FUNCTION IF EXISTS stms_rates_by_connectiondetails_id( bigint, bigint, date);
...
BEGIN
	...
	select extract(epoch from executiondate) into v_executiondate from egswtax_connection where shsc_number = in_consumercode and status='ACTIVE';
	...
	if (v_executiondate is null) then
		if (batch_id is null or batch_id = 0) then
			raise exception 'Connection is not executed: %',in_consumercode ;
		else
			v_status := false;
			insert into egswtax_batchjobdetailslog(id, jobdetails, consumercode, status, reason, createddate, lastmodifieddate, createdby, lastmodifiedby) values(nextval('seq_egstax_batchjobdetailslog'), batch_id, in_consumercode, 'Failure', 'Connection is not executed', now(), now(), (select id from eg_user where username='egovernments'), (select id from eg_user where username='egovernments'));
			return -1;
		end if;
	end if;
	...
	if ( (v_curr_inst_todate-v_executiondate) < 0) then
		if (batch_id is null or batch_id = 0) then
			raise exception 'Billing cycle period is after the execution date for consumer: %',in_consumercode ;
		else
			v_status := false;
			insert into egswtax_batchjobdetailslog(id, jobdetails, consumercode, status, reason, createddate, lastmodifieddate, createdby, lastmodifiedby) values(nextval('seq_egstax_batchjobdetailslog'), batch_id, in_consumercode, 'Failure', 'Billing cycle period is after the execution date', now(), now(), (select id from eg_user where username='egovernments'), (select id from eg_user where username='egovernments'));
			return -1;
		end if;
	end if;
...
BEGIN
	...
	if(v_wateridentifier is not null) then
		...
		if (v_wt_conn_det_id is null) then
		select cd.id,cd.billingtype into v_wt_conn_det_id,v_wt_billingtype from egwtr_connectiondetails cd,egwtr_connection c where c.id = cd.connection and c.consumercode = v_wateridentifier;
		end if;
	...
	-- Anandpur Sahib, if sewerage plot >500, then take water demand generated
	if (v_citycode = '2001' and v_plotsize > 500) then
		v_ratetype := 'SEWERAGE_EQUAL_TO_WATER';
	end if;
	...
	if(v_ratetype is not null and ( v_ratetype = 'PLOT_BASED' or v_ratetype = 'SEAT_BASED' )) then
		...
		if(v_billingtype is not null and v_billingtype = 'CUSTOM') then
			...
			if(v_ratetype is not null and v_ratetype = 'FIXED') then
				select fixedCharges into v_rate from egswtax_seweragecharge_rates where usagetype = v_usagetype and ratetype = 'FIXED' and effectiveFromDate<=in_curr_instl_end_date and effectiveToDate>=in_curr_instl_end_date ;
				raise notice 'Got data from rates master by fixed is:%',v_rate;
			end if;

			if(v_ratetype is not null and v_ratetype = 'SEAT_BASED') then
				for rates in (select fromnoofseats, tonoofseats, rateAmount,fixedCharges from egswtax_seweragecharge_rates where usagetype = v_usagetype and effectiveFromDate<=in_curr_instl_end_date and effectiveToDate>=in_curr_instl_end_date)
				loop
					begin
						if(rates.fromnoofseats is not null and rates.fromnoofseats <= v_numberofclosets and rates.tonoofseats is not null and rates.tonoofseats <= v_numberofclosets and rates.fixedCharges is null) then
						...
						if(rates.fromnoofseats is not null and rates.fromnoofseats <= v_numberofclosets and rates.tonoofseats is null and rates.fixedCharges is null) then
							v_diff := ( v_numberofclosets - rates.fromnoofseats) + 1;
							v_temprate := v_temprate + (v_diff * rates.rateAmount);
						end if;
						if(rates.fromnoofseats is null and rates.tonoofseats is null and rates.fixedCharges is not null) then
...
CREATE OR REPLACE FUNCTION stms_demand_generation_titletransfer_for_consumer_code(in_consumercode varchar, in_idappdetail bigint, in_curr_inst_id bigint, in_prev_inst_id bigint, batch_id bigint,applicationdate date)
RETURNS numeric AS
$BODY$
DECLARE
v_olddemandid bigint;
...
BEGIN
	...
	raise notice 'stms_demand_generation_titletransfer_for_consumer_code, in_consumercode, in_idappdetail, in_curr_inst_id, in_prev_inst_id % % % %',in_consumercode, in_idappdetail, in_curr_inst_id, in_prev_inst_id;
	
	--validation for application date validation starts select 

v_applicationdate:=extract(epoch from applicationdate);

select executiondate into v_executiondate from egswtax_connection where shsc_number = in_consumercode AND status='ACTIVE' ;
...
select id into v_exceution_date_instllid  from eg_installment_master where installment_type='Quarterly' and  start_date <= v_executiondatetime and end_date >=v_executiondatetime and id_module=(select id from eg_module where name ='Sewerage Tax Management');
	...
	if (v_applicationdate is null) then
		if (batch_id is null or batch_id = 0) then
			raise exception 'Sewerage Connection is not applied: %',in_consumercode ;
		else
			v_status := false;
			insert into egswtax_batchjobdetailslog(id, jobdetails, consumercode, status, reason, createddate, lastmodifieddate, createdby, lastmodifiedby) values(nextval('seq_egstax_batchjobdetailslog'), batch_id, in_consumercode, 'Failure', 'Connection has not been applied', now(), now(), (select id from eg_user where username='egovernments'), (select id from eg_user where username='egovernments'));
		end if;
	end if;

	if ( (v_curr_inst_todate-v_applicationdate) < 0) then
		if (batch_id is null or batch_id = 0) then
			raise exception 'For Sewerage Connection,Billing cycle period is after the application date for consumer: %',in_consumercode ;
		else
			v_status := false;
			insert into egswtax_batchjobdetailslog(id, jobdetails, consumercode, status, reason, createddate, lastmodifieddate, createdby, lastmodifiedby) values(nextval('seq_egstax_batchjobdetailslog'), batch_id, in_consumercode, 'Failure', 'Billing cycle period is after the execution date', now(), now(), (select id from eg_user where username='egovernments'), (select id from eg_user where username='egovernments'));
		end if;
	...
	--validation for application date validation ends


	--validation for sequence of billing cycles starts
	select inst.id into v_connlatestinstid from eg_demand d, egswtax_demand_connection dc, eg_installment_master inst, eg_demand_details dd, eg_demand_reason_master dm, eg_demand_reason dr where dc.applicationdetail=in_idappdetail and dc.demand=d.id and inst.id=d.id_installment and d.id=dd.id_demand and dd.id_demand_reason=dr.id and dr.id_demand_reason_master=dm.id and dm.code='SEWERAGETAX' and d.is_history='N' order by inst.start_date desc limit 1;
	 select isexempted into v_isexempted from egswtax_connectiondetail where id = (select connectiondetail from egswtax_applicationdetails where connection in (select id from egswtax_connection where shsc_number =in_consumercode) and isactive=true);

	if(v_connlatestinstid is null and v_exceution_date_instllid != in_curr_inst_id and v_isexempted != true) then
if (batch_id is null or batch_id = 0) then
			raise exception 'Please generate previous quarter Sewerage demand for : %',in_consumercode ;
		else
			v_status := false;
	...
	if(v_connlatestinstid is not null and v_connlatestinstid = in_curr_inst_id) then
		if (batch_id is null or batch_id = 0) then
			raise exception 'For Sewerage Connection,Demand is already generated for given billing cycle: %',in_consumercode ;
		else
			v_status := false;
			insert into egswtax_batchjobdetailslog(id, jobdetails, consumercode, status, reason, createddate, lastmodifieddate, createdby, lastmodifiedby) values(nextval('seq_egstax_batchjobdetailslog'), batch_id, in_consumercode, 'Failure', 'Demand is already generated for given billing cycle', now(), now(), (select id from eg_user where username='egovernments'), (select id from eg_user where username='egovernments'));
		end if;
	...
	if(v_instdiffinsecs>1)then
		raise notice 'The difference between end date of previous billing cycle and start date of current billing cycle provide is (in secs) %',v_instdiffinsecs;
		if (batch_id is null or batch_id = 0) then
			raise exception 'For Sewerage Connection,Demand generation is not allowed as previous bill/demand are not generated';
		else
			v_status := false;
			insert into egswtax_batchjobdetailslog(id, jobdetails, consumercode, status, reason, createddate, lastmodifieddate, createdby, lastmodifiedby) values(nextval('seq_egstax_batchjobdetailslog'), batch_id, in_consumercode, 'Failure', 'Demand generation is not allowed as previous bill/demand are not generated', now(), now(), (select id from eg_user where username='egovernments'), (select id from eg_user where username='egovernments'));
			end if;
	--validation for sequence of billing cycles ends
	else
	select min(d.id) into v_firstdemandid from eg_demand d, egswtax_demand_connection dc where dc.applicationdetail=in_idappdetail and dc.demand=d.id;
	...
	if(v_prev_bill_id is null) then
		select description into v_sc_prev_billingcycle from eg_installment_master where id=in_prev_inst_id;
		select id into v_wc_prev_inst_id from eg_installment_master where description = v_sc_prev_billingcycle and id_module = (select id from eg_module where name='Sewerage Tax Management');
		select bill.id into v_prev_bill_id from egwtr_connection wconn, egwtr_connectiondetails wcd, egswtax_connection sconn, egswtax_connectiondetail scd, egswtax_applicationdetails sad, egwtr_demand_connection wdc, eg_demand d, eg_bill bill where sconn.id=sad.connection and scd.id=sad.connectiondetail and scd.wateridentifier=wconn.oldconsumernumber and wcd.connection=wconn.id and wcd.id=wdc.connectiondetails and wdc.demand=d.id and d.id=bill.id_demand and bill.is_cancelled='N' and bill.id_bill_type=(select id from eg_bill_type where code='INTEGRATED') and d.id_installment=v_wc_prev_inst_id and sconn.shsc_number=in_consumercode;
	...
	raise notice 'stms_demand_generation_titletransfer_for_consumer_code,  v_olddemandid, v_currdemandid, v_prev_bill_id % % %',v_olddemandid, v_currdemandid, v_prev_bill_id;

	if(v_currdemandid is null) then
		if((v_olddemandid is not null and v_prev_bill_id is not null) or (v_olddemandid is null)) then
			raise notice 'stms_demand_generation_titletransfer_for_consumer_code, v_olddemandid is null';
			select end_date into v_curr_instl_end_date from eg_installment_master where id = in_curr_inst_id;
			raise notice 'stms_demand_generation_titletransfer_for_consumer_code, v_curr_instl_end_date % ', v_curr_instl_end_date;
			...
			v_conn_date:= applicationdate;
				...
				if (v_curr_instl_start_date <= v_conn_date and v_curr_instl_end_date >= v_conn_date and v_curr_instl_start_date >= v_executiondate) then
					...
					v_demandAmountfromSWRate := round(v_days_btw_conn_date * (v_demandAmountfromSWRate / v_days_btw_quarter));
	...
	--exempted case validation starts
		if(v_demandAmountfromSWRate is not null and v_demandAmountfromSWRate = 0) then
			if (batch_id is null or batch_id = 0) then
				raise exception 'Sewerage Connection with consumer number: % is exempted. Hence the sewerage charges is zero.',in_consumercode ;				else
				v_status := false;
				insert into egswtax_batchjobdetailslog(id, jobdetails, consumercode, status, reason, createddate, lastmodifieddate, createdby, lastmodifiedby) values(nextval('seq_egstax_batchjobdetailslog'), batch_id, in_consumercode, 'Success', 'Connection with consumer number: ' || in_consumercode || ' is exempted. Hence the sewerage charges is zero.', now(), now(), (select id from eg_user where username='egovernments'), (select id from eg_user where username='egovernments'));
			end if;
	...
	--exempted case validation ends
			...
			if(v_demandAmountfromSWRate is not null and v_demandAmountfromSWRate > 0) then
				...
				if (v_olddemandid IS NOT NULL) then 
					...
					insert into eg_demand (id, id_installment, base_demand, is_history, create_date, modified_date, amt_collected, status, min_amt_payable, amt_rebate) select v_newdemandid, in_curr_inst_id, base_demand, is_history, now(), now(),amt_collected, status, null, 0 from eg_demand where id = v_olddemandid;
					--copying old demand_demand_details to new eg_demand.
					raise notice 'stms_demand_generation_titletransfer_for_consumer_code, copying old demand_demand_details to new eg_demand.';
					insert into eg_demand_details(id, id_demand, id_demand_reason, id_status, file_reference_no, remarks, amount, modified_date, create_date, amt_collected, amt_rebate) select nextval('seq_eg_demand_details'), v_newdemandid, id_demand_reason, id_status, file_reference_no, remarks, amount, modified_date, create_date, amt_collected, amt_rebate from eg_demand_details where id_demand = v_olddemandid and id_demand_reason not in (select id from eg_demand_reason where id_demand_reason_master = (select id from eg_demand_reason_master where code = 'SEWERAGEADVANCE'));
					raise notice 'copied old demand details to new eg_demand for consumer code: %',in_consumercode;
				...
				if (batch_id is null or batch_id = 0) then
				raise exception 'Sewerage Bill is not generated for previous billing cycle: %',in_consumercode ;			else
				v_status := false;
				insert into egswtax_batchjobdetailslog(id, jobdetails, consumercode, status, reason, createddate, lastmodifieddate, createdby, lastmodifiedby) values(nextval('seq_egstax_batchjobdetailslog'), batch_id, in_consumercode, 'Failure', 'Bill is not generated for previous billing cycle', now(), now(), (select id from eg_user where username='egovernments'), (select id from eg_user where username='egovernments'));
			end if;
			end if;

...
begin
	...
	if(v_billdate is not null) then
		...
		if(v_penalty_date < now() and v_previousChargesBal>0) then
			...
			select id into v_penalty_demand_reason_id from eg_demand_reason where id_demand_reason_master in(select id from eg_demand_reason_master where code='PENALTY' and module = v_moduleid) and id_installment=in_curr_inst_id;
		...
		if(v_interest_date < now() and v_previousChargesBal>0) then
			...
			select id into v_interest_demand_reason_id from eg_demand_reason where id_demand_reason_master in(select id from eg_demand_reason_master where code='INTEREST' and module = v_moduleid) and id_installment=in_curr_inst_id;

...
BEGIN
	...
	select count(dcb.oldconsumernumber) into v_wrongdcbcount from egswtax_stg_connection_dcb dcb where (coalesce(chargeamount, 0)-coalesce(chargecollection, 0)) + (coalesce(penaltyamount, 0)-coalesce(penaltycollection, 0)) + (coalesce(interestamount, 0)-coalesce(interestcollection, 0))>0 and coalesce(advancecollection, 0)>0;
	...
	if (v_noparentboundaries = 0 and v_noofownernull = 0 and v_nooflocalitynull = 0 and v_noofpincodenull = 0 and v_noofoldconsumernumbernull = 0 and v_noofconnectiondatenull=0 and v_noofusagetypenull = 0 and v_duplicatconnection = 0 and v_wrongdcbcount = 0) then
	...
	select count(*) into v_duplicatebillingcycle from egswtax_stg_connection_dcb group by oldconsumernumber, billingcycle having count(*) > 1;

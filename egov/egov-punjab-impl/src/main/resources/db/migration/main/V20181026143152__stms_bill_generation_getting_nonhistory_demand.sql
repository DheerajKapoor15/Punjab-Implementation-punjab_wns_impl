...
begin
	...
	if(v_currdemand is not null) then
		...
		select bill.id into v_water_intgbill from eg_bill bill, egwtr_connection wcon, egswtax_connection scon, egwtr_connectiondetails wcdet, egwtr_demand_connection wdcn, eg_demand dm, eg_installment_master mst where scon.oldconsumernumber = wcon.sewerageidentifier and wcon.consumercode = bill.consumer_id and bill.id_bill_type = (select id from eg_bill_type where code='INTEGRATED') and wcon.id=wcdet.connection and wcdet.id=wdcn.connectiondetails and wdcn.demand=dm.id and dm.is_history='N' and dm.id_installment=mst.id and mst.description=v_cur_description and mst.id_module = (select id from eg_module where name='Water Tax Management') and bill.is_cancelled='N' and scon.shsc_number = in_consumercode and bill.module_id = (select id from eg_module where name='Water Tax Management');

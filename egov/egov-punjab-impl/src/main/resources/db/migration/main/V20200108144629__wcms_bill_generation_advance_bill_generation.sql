...
begin
	...
	if (in_bill_type='INTEGRATED') then
		select app.id into v_stms_appdet from egswtax_applicationdetails app, egswtax_connection con where app.connection=con.id and con.oldconsumernumber=(select sewerageidentifier from egwtr_connection where consumercode=in_consumercode);
		if (v_stms_appdet is null) then
			select app.id into v_stms_appdet from egswtax_applicationdetails app, egswtax_connection con where app.connection=con.id and con.shsc_number=(select sewerageidentifier from egwtr_connection where consumercode=in_consumercode);
		end if;

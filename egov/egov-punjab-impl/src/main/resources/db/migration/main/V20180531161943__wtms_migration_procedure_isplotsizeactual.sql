	...
	if (v_isdatavalid) then
		...
		loop 
			begin
				...
				v_temp  := pbwc_migrateConnection(connections.oldconsumernumber, connections.connectiondate, v_currinst, v_currinstdesc, v_moduleid, connections.corraddress);
			EXCEPTION
			WHEN OTHERS THEN
...
CREATE OR REPLACE FUNCTION pbwc_migrateConnection(oldConnectionNo IN varchar(60), connectionDate IN date, currinst IN bigint, currinstdesc IN varchar(16), idModule IN bigint, corrAddress IN varchar(256))
RETURNS bigint AS
$BODY$
DECLARE
...
BEGIN 
	...
	select houseno, 'WTCONNECTION_ADDRESS', landmark, street, (select name from eg_boundary where id=locality::bigint), pincode, v_addresspk, userid, buildingname from egwtr_stg_connection where oldconsumernumber = oldConnectionNo;
...
INSERT INTO egwtr_connection(id, oldconsumernumber, consumercode, propertyidentifier, address, sewerageidentifier, zone, ward, block, locality, createdby, createddate, lastmodifieddate, lastmodifiedby, version)
	select v_connectionpk, oldconsumernumber, v_connectionno, propertyidentifier, addressid, sewerageconnectionnumber, null, null, null, (select id from eg_boundary where id=locality::bigint), 1, connectiondate, connectiondate, 1, 0 from egwtr_stg_connection where oldconsumernumber = oldConnectionNo;
...
BEGIN
	...
	if (v_noofownernull = 0 and v_noofhousenonull = 0 and v_nooflocalitynull = 0 and v_noofpincodenull = 0 and v_noofoldconsumernumbernull = 0 and v_noofconnectiondatenull=0 and v_noofconnectiontypenull = 0 and v_noofwatersourcenull = 0 and v_noofpropertytypenull = 0 and v_noofcategorynull = 0 and v_noofusagetypenull = 0 and v_noofpipesizenull = 0) then
	...
	select count(*) into v_invalidwatersource from egwtr_stg_connection where watersource not in ('GROUND_WATER');

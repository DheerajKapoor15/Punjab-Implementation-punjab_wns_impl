DROP FUNCTION IF EXISTS wtms_bill_genaration_for_non_metered_connections(batch_name varchar,boundary_type varchar,curr_installment_description varchar, batch_id bigint);

DROP FUNCTION IF EXISTS wtms_bill_genaration_for_boundary(boundary_type varchar, in_location_id bigint, in_curr_inst_id bigint, in_prev_inst_id bigint, curr_installment_description varchar, batch_id bigint);
...
begin
	...
	select id into v_moduleid from eg_module where name ='Water Tax Management';
	select id into v_idbillype from eg_bill_type where code=in_bill_type;
	select id into v_conn_det_id from egwtr_connectiondetails where connectionstatus='ACTIVE' and  connection in(select id from egwtr_connection where consumercode=in_consumercode);

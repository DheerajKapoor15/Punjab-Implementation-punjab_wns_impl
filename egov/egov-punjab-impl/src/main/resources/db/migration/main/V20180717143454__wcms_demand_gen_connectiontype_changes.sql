DROP FUNCTION IF EXISTS wtms_demand_genaration(connection_type varchar, batch_name varchar,boundary_type varchar,curr_installment_description varchar, batch_id bigint);
DROP FUNCTION IF EXISTS wtms_demand_genaration_for_boundary(connection_type varchar,boundary_type varchar, in_location_id bigint, in_curr_inst_id bigint, in_prev_inst_id bigint, batch_id bigint);
DROP FUNCTION IF EXISTS wtms_demand_generation_for_consumer_code(connection_type varchar,in_consumer_code varchar,in_idconndetail bigint, in_curr_inst_id bigint, in_prev_inst_id bigint, batch_id bigint);
...
BEGIN
	...
	if(v_connectiontype is not null and v_connectiontype = v_connectiontypeid and v_billingtype is not null and v_billingtype = 'STANDARD' ) then
		...
		select unitsconsumed,meterstatus into v_unitsconsumed,v_meterstatus from egwtr_meter_reading_details where connectionDetails=conn_detail_id and installment=in_curr_inst_id;

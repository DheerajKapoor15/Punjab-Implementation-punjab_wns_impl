DROP FUNCTION if exists wtms_penalty_and_interest_generate_for_consumercode(bigint,bigint,bigint,varchar,bigint,bigint,bigint,bigint);
DROP FUNCTION if exists wtms_bill_genaration_for_consumer_code(varchar(16), bigint, bigint, bigint, bigint, varchar, bigint,varchar,bigint);
DROP FUNCTION if exists wtms_bill_detail_genaration_for_consumer_code(bigint, bigint, bigint, bigint, bigint, bigint, varchar);
DROP FUNCTION if exists stms_bill_detail_genaration_for_water(varchar,bigint,bigint,varchar,varchar);
DROP FUNCTION if exists wtms_bill_genaration(bigint, varchar, varchar, varchar, bigint);
DROP FUNCTION if exists wtms_bill_genaration_for_boundary(bigint, varchar, bigint, bigint, bigint, varchar, bigint);
DROP FUNCTION if exists stms_penalty_and_interest_generate_for_consumercode(varchar,varchar,bigint,bigint,varchar);


CREATE OR REPLACE FUNCTION wtms_bill_genaration(connection_type bigint, batch_name varchar, boundary_type varchar, curr_installment_description varchar, batch_id bigint, bill_type varchar)
RETURNS void AS
$BODY$
DECLARE
props record;
...
BEGIN
	...
	raise notice 'v_curr_inst_id, v_curr_inst_id_temp % %', v_curr_inst_id, v_curr_inst_id_temp;
		...
		loop 
			...
			v_temp := wtms_bill_genaration_for_boundary(connection_type,boundary_type, props.id,v_curr_inst_id, v_prev_inst_id, curr_installment_description, batch_id, bill_type);
...
create or replace function wtms_bill_genaration_for_boundary(connection_type bigint,boundary_type varchar, in_location_id bigint, in_curr_inst_id bigint, in_prev_inst_id bigint, curr_installment_description varchar, batch_id bigint, bill_type varchar)
returns numeric as 
$BODY$
declare
rec record;
	  	...
	  	loop
			begin
				raise notice 'Started bill generation for consumercode: %',rec.consumercode;
	    			...
	    			v_temp := wtms_bill_genaration_for_consumer_code(rec.consumercode, rec.connid, rec.conndetid, in_curr_inst_id, in_prev_inst_id, curr_installment_description, rec.locationid, bill_type, batch_id);
...
v_is_connection_individual boolean;
v_connection_bill_type varchar;
v_seweragedemand bigint;
v_stms_appdet bigint;
billinstallment bigint;
currinstall_enddate date;

begin
	select id into v_moduleid from eg_module where name='Water Tax Management';

	-- Integrated bill for individual connection validation starts
	v_connection_bill_type := getbilltype_for_bill_generation(in_consumercode);
	if (v_connection_bill_type='MANUAL') then
		v_is_connection_individual:= true;
	else 
		v_is_connection_individual:= false;
	end if;
	raise notice 'eg_bill created for consumer number %', v_is_connection_individual;
	if (v_is_connection_individual=true and  in_bill_type='INTEGRATED') then
		if (batch_id is null or batch_id = 0) then
			raise exception 'Cannot generate integrated bill for non integrated connections';
		else
			v_status := false;
			insert into egwtr_batchjobdetaiapplicationdetailslslog(id, jobdetails, consumercode, status, reason, createddate, lastmodifieddate, createdby, lastmodifiedby) values(nextval('seq_egwtr_batchjobdetailslog'), batch_id, in_consumercode, 'Failure', 'Cannot generate integrated bill for non integrated connections', now(), now(), (select id from eg_user where username='egovernments'), (select id from eg_user where username='egovernments'));
			return -1;
		end if; 	
	end if;
	-- Integrated bill for individual connection validation ends

	--Integrated bill sewerage demand generated validation starts
	if (in_bill_type='INTEGRATED') then
		select app.id into v_stms_appdet from egswtax_applicationdetails app, egswtax_connection con where app.connection=con.id and con.oldconsumernumber in (select sewerageidentifier from egwtr_connection where consumercode=in_consumercode) and con.status='ACTIVE';
		if (v_stms_appdet is null) then
			select app.id into v_stms_appdet from egswtax_applicationdetails app, egswtax_connection con where app.connection=con.id and con.shsc_number in (select sewerageidentifier from egwtr_connection where consumercode=in_consumercode)and con.status='ACTIVE';
			
		end if;


		select id into v_seweragedemand from eg_demand where id in (select demand from egswtax_demand_connection where applicationdetail=v_stms_appdet) and is_history='N' and id_installment = (select id from eg_installment_master where id_module=(select id from eg_module where name = 'Sewerage Tax Management') and description=curr_installment_description and installment_type ='Quarterly');

		if (v_status and v_seweragedemand is null) then
			if (batch_id is null or batch_id = 0) then
				raise exception 'Sewerage Demand is not generated for the integrated connection';
			else
				v_status := false;
				insert into egwtr_batchjobdetailslog(id, jobdetails, consumercode, status, reason, createddate, lastmodifieddate, createdby, lastmodifiedby) values(nextval('seq_egwtr_batchjobdetailslog'), batch_id, in_consumercode, 'Failure', 'Sewerage Demand is not generated for the integrated connection', now(), now(), (select id from eg_user where username='egovernments'), (select id from eg_user where username='egovernments'));
				return -1;
			end if; 	
		end if;	
	end if;
	--Integrated bill sewerage demand generated validation ends

	select conndet.executiondate into v_executiondate from egwtr_connection conn, egwtr_connectiondetails conndet where conndet.connection=conn.id and conn.consumercode=in_consumercode and conndet.connectionstatus='ACTIVE';
	...
	if(v_status and v_isexedateincurrent=false and v_executiondate>v_currinststartdate)then
		if (batch_id is null or batch_id = 0) then
	...
	select bill.id_demand into v_latestbilldemandid from eg_bill bill, eg_demand d where d.id=bill.id_demand and bill.module_id=v_moduleid and bill.is_cancelled='N' and bill.id_bill_type != (select id from eg_bill_type where code = 'AUTO') and bill.consumer_id=in_consumercode order by bill.issue_date desc limit 1;
	...
	if(v_status = true and v_instdiffinsecs<0)then
	...
	if(v_currdemand is not null) then
		...
		if (in_bill_type= 'INTEGRATED') then
			...
			if (v_sewerage_bill_exists is null) then

				select bill.id into v_sewerage_bill_exists from eg_bill bill, eg_installment_master mst, egwtr_connection wcon, egswtax_connection scon, egswtax_applicationdetails sapp, egswtax_demand_connection sdmc, eg_demand dm where scon.shsc_number = wcon.sewerageidentifier and scon.shsc_number = bill.consumer_id and bill.id_bill_type = (select id from eg_bill_type where code='MANUAL') and scon.id=sapp.connection and sapp.id=sdmc.applicationdetail and sdmc.demand=dm.id and dm.is_history='N' and bill.id_demand=dm.id and dm.id_installment=mst.id and mst.description=v_cur_description and mst.id_module = (select id from eg_module where name='Sewerage Tax Management') and bill.is_cancelled='N' and wcon.consumercode = in_consumercode and bill.module_id = (select id from eg_module where name='Sewerage Tax Management');
			end if;	
		...
		if(v_demandbill is null and v_sewerage_bill_exists is null) then
			...
			select code into citycode from eg_city;
	 		select citycode || lpad(nextval('SEQ_EG_MANUAL_INTEGRATED_BILL')::text,11,'0') into v_billno;
			...
			select id into billinstallment  from eg_installment_master where start_date <=now() and end_date>=now() and id_module=(select id from eg_module where name='Water Tax Management');
			select end_date into currinstall_enddate from eg_installment_master where id=in_curr_inst_id;
			if(billinstallment != in_curr_inst_id) then
			...
			insert into eg_bill(id, id_demand, citizen_name, citizen_address, bill_no, id_bill_type, issue_date, last_date, module_id, user_id, create_date, modified_date, is_history, is_cancelled, fundcode, functionary_code, fundsource_code, department_code, coll_modes_not_allowed, boundary_num, boundary_type, total_amount, total_collected_amount, service_code, part_payment_allowed, override_accountheads_allowed, description, min_amt_payable, consumer_id, dspl_message, callback_for_apportion, penalty) values (v_billpk, v_currdemand, v_ownername, v_houseno, v_billno, v_idbillype, now(), currinstall_enddate+INTERVAL '1 DAY'*v_penalty_period, v_moduleid, v_iduser, now(), now(), 'N', 'N', v_fund_code, v_functionary_code, v_fundsource_code, v_dept_code, null, in_location_id, null, v_totalamount, v_totalamountcollected, 'WT', 'Y', 'N', v_description, 0, in_consumercode, 'Water Charge Collection', 'N', v_penalty);
		end if;
...
--Sewerage integration
			...
			if (in_bill_type= 'INTEGRATED') then
				...
				select app.id into v_stms_appdetid from egswtax_applicationdetails app, egswtax_connection con where app.connection=con.id and con.oldconsumernumber in (select sewerageidentifier from egwtr_connection where consumercode=in_consumercode) and con.status='ACTIVE';

				if (v_stms_appdetid is null) then
					select app.id into v_stms_appdetid from egswtax_applicationdetails app, egswtax_connection con where app.connection=con.id and con.shsc_number in (select sewerageidentifier from egwtr_connection where consumercode=in_consumercode) and con.status='ACTIVE';
				end if;
			...
			if (batch_id is null or batch_id = 0) then
					raise exception '% bill already present for this consumer number: %', v_billname, in_consumercode ;			
		...
		if (batch_id is null or batch_id = 0) then
			raise exception 'Demand does not exist for current installment of Consumer No %',in_consumercode ;			
		elsif (v_status = true) then
			v_status := false;
...
v_rent_glcode character varying(50);
v_rent_glcodeid bigint;
v_connection_type character varying(30);
v_rentarrears_amount double precision;
v_citycode character varying(10);
...
BEGIN
	...
	select code into v_connection_type from egwtr_connection_type where id=(select connectiontype from egwtr_connectiondetails where id=in_conn_det_id);
	...
	select id,glcodeid into v_rent_demandreason, v_rent_glcodeid from eg_demand_reason where id_demand_reason_master in (select id from eg_demand_reason_master where code='METERRENT' and module = v_module_id) and id_installment=in_curr_inst_id;
	...
	select glcode into v_rent_glcode from chartofaccounts where id = v_rent_glcodeid;
	...
	select dd.amount into v_bd_penalty from eg_demand_details dd where dd.id_demand=in_demandid and dd.amount>0 and dd.id_demand_reason=v_bd_penalty_demandreason ;
	...
	if(v_bd_penalty is not null and v_bd_penalty>0) then
	...
	if(v_connection_type='METERED' and v_citycode='1407' ) then
	-- rent amount
	select dd.amount into v_rent_amount from eg_demand_details dd where dd.id_demand=in_demandid and dd.amount>0 and dd.id_demand_reason=v_rent_demandreason ;
	
	insert into eg_bill_details(id, id_demand_reason, create_date, modified_date,id_bill, collected_amount, order_no, glcode, function_code, cr_amount, dr_amount, description, id_installment, additional_flag,purpose) values(nextval('seq_eg_bill_details'), v_rent_demandreason, now(), now(), v_billpk, 0, 2, v_rent_glcode, v_function_code, v_rent_amount, 0, v_billDesc, in_curr_inst_id, 1, 'METERRENT');

	
	--rent arrears
	select sum(dd.amount - dd.amt_collected) into v_rentarrears_amount from eg_demand_details dd,eg_demand_reason dr, eg_demand_reason_master drm where dd.id_demand_reason=dr.id and dd.id_demand_reason = dr.id and dr.id_demand_reason_master = drm.id and drm.code = 'METERRENT' and drm.module = v_module_id and dr.id_installment not in (in_curr_inst_id) and dd.id_demand=in_demandid ;

	if(v_rentarrears_amount is not null and v_rentarrears_amount > 0) then 
		insert into eg_bill_details(id, id_demand_reason, create_date, modified_date,id_bill, collected_amount, order_no, glcode, function_code, cr_amount, dr_amount, description, id_installment, additional_flag,purpose) values(nextval('seq_eg_bill_details'), v_rent_demandreason, now(), now(), v_billpk, 0, 2, v_rent_glcode, v_function_code, v_rentarrears_amount, 0, v_billDesc, in_curr_inst_id, 1, 'METERRENTARREARS');
	end  if;
	end if;
...
v_interest_demand_detail	bigint;
v_connection_type varchar;
v_citycode character varying(10);
v_previousChargeMeterRent double precision;
v_previousChargesMeterRentBal double precision;

begin
	v_penalty := 0;
	v_previousChargeMeterRent :=0;
	v_previousChargesMeterRentBal :=0;
	...
	select last_date into v_penalty_date from eg_bill where consumer_id=in_consumercode and module_id = in_moduleid and id_bill_type != (select id from eg_bill_type where code='AUTO') and is_cancelled = 'N' and id_demand=in_previousdemandid;

	select sum(dd.amount), sum(dd.amount-dd.amt_collected) into v_previousCharge, v_previousChargesBal from eg_demand_details dd, eg_demand_reason dr, eg_demand_reason_master drm where dd.id_demand=in_currdemand and dd.id_demand_reason=dr.id and drm.id=dr.id_demand_reason_master and dr.id_installment=in_prev_inst_id and drm.code='WTAXCHARGES' and drm.module = in_moduleid;
	
	select code into v_connection_type from egwtr_connection_type where id=(select connectiontype from egwtr_connectiondetails where id=in_conn_det_id);
	select code into v_citycode from eg_city where active = true;
	
	if(v_connection_type='METERED' and v_citycode='1407') then
	select sum(dd.amount), sum(dd.amount-dd.amt_collected) into v_previousChargeMeterRent, v_previousChargesMeterRentBal from eg_demand_details dd, eg_demand_reason dr, eg_demand_reason_master drm where dd.id_demand=in_currdemand and dd.id_demand_reason=dr.id and drm.id=dr.id_demand_reason_master and dr.id_installment=in_prev_inst_id and drm.code='METERRENT' and drm.module = in_moduleid;
	else
	v_previousChargeMeterRent=0;
	v_previousChargesMeterRentBal=0;
	end if;
	
	if(v_previousChargeMeterRent is null) then
	  v_previousChargeMeterRent=0;
	  			end if;

	  if(v_previousChargesMeterRentBal is null) then
	  v_previousChargesMeterRentBal=0;
	  			end if;
	...
	if(v_billdate is not null) then
		...
		select 	v_penalty_date + INTERVAL '1 DAY' * (v_interest_period) into v_interest_date;
		...
		if(v_penalty_date < now() and ((v_previousChargesBal>0) or (v_previousChargesMeterRentBal>0))) then
			v_penalty := ((v_previousCharge * v_penalty_percentage)/100);
			v_penalty:=(v_penalty + (v_previousChargeMeterRent * v_penalty_percentage)/100);
			v_penalty:=ROUND(v_penalty)::numeric;
		...
		if(v_interest_date < now() and v_previousChargesBal>0) then
			...
			select id into v_interest_demand_detail from eg_demand_details where id_demand_reason=v_interest_demand_reason_id and id_demand=in_currdemand;
			if(v_interest_demand_detail is null and v_interest>0) then
				...
				update eg_demand_details set amount=v_interest where id=v_interest_demand_detail;
			end if;
...
BEGIN
		select sewerageidentifier into v_stms_identifier from egwtr_connection where id=(select connection from egwtr_connectiondetails  where connection in (select id from egwtr_connection where consumercode=in_consumercode) and connectionstatus='ACTIVE');
...
--penalty calculation
	...
	-- penalty amount
		...
		if(v_penalty_amount is not null and v_penalty_amount > 0) then 
			insert into eg_bill_details(id, id_demand_reason, create_date, modified_date,id_bill, collected_amount, order_no, glcode, function_code, cr_amount, dr_amount, description, id_installment, additional_flag,purpose) values(nextval('seq_eg_bill_details'), v_penalty_demandreason, now(), now(), v_billpk, 0, 2, 	v_penalty_glcode, v_function_code, v_penalty_amount, 0, v_billDesc, v_curr_inst_id, 1,'ARREAR_PENALTY_CHARGES');
		end  if;
...
begin
	...
	select id into v_wmoduleid from eg_module where name ='Water Tax Management';
	select id into v_idbillype from eg_bill_type where code=in_bill_type;
	...
	select last_date into v_penalty_date from eg_bill where consumer_id=in_sconsumercode and module_id = v_moduleid and id_bill_type != (select id from eg_bill_type where code='AUTO') and is_cancelled = 'N' and id_demand=v_previousdemandid;
	...
	if(v_billdate is null)then
		...
		select issue_date into v_billdate from eg_bill where consumer_id=in_wconsumercode and module_id = v_wmoduleid and id_bill_type != (select id from eg_bill_type where code='AUTO') and is_cancelled = 'N' and id_demand=v_previouswaterdemandid;
		select last_date into v_penalty_date from eg_bill where consumer_id=in_wconsumercode and module_id = v_wmoduleid and id_bill_type != (select id from eg_bill_type where code='AUTO') and is_cancelled = 'N' and id_demand=v_previouswaterdemandid;
	...
	if(v_billdate is not null) then
		...
		if(v_interest_date < now() and v_previousChargesBal>0) then
			...
			select id into v_interest_demand_detail from eg_demand_details where id_demand_reason=v_interest_demand_reason_id and id_demand=v_currdemandid;
			if(v_interest_demand_detail is null and v_interest>0) then

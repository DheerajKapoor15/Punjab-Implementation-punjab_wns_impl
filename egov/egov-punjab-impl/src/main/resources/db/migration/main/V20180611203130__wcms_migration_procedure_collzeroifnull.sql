...
BEGIN
	...
	if (v_isdatavalid) then
		...
		select id, description into v_currinst, v_currinstdesc from eg_installment_master where id_module=v_moduleid and (select (end_date - INTERVAL '1 DAY') from eg_installment_master where id_module=v_moduleid and now() between start_date and end_date) between start_date and end_date;

		for connections in (select * from egwtr_stg_connection where (ismigrated='N' or ismigrated is null))
		loop 
			begin
...
BEGIN 
	...
	loop 
		...
		v_temp := pbwc_createInstDemandDetail(v_demandpk, v_idinst, connectionDCB.chargeamount, connectionDCB.chargecollection, connectionDCB.penaltyamount, connectionDCB.penaltycollection, connectionDCB.interestamount, connectionDCB.interestcollection, idModule, connectiondate);
	END LOOP;
return v_demandpk;
EXCEPTION
WHEN OTHERS THEN
...
create or replace FUNCTION pbwc_createInstDemandDetail(idDemand IN bigint, idInstall IN bigint, chargeAmt IN double precision, chargeColl IN double precision, penaltyAmt IN double precision, penaltyColl IN double precision, interestAmt IN double precision, interestColl IN double precision, idModule IN bigint, connectionDate IN date) 
RETURNS numeric as $$
declare
	v_temp 	bigint;  
BEGIN 
...
BEGIN 
	...
	VALUES (v_dmddetpk, idDemand, idDemandRsn, taxAmount, coalesce(collAmount, 0), connectiondate, connectiondate);
...
declare
	...
	propType egwtr_stg_propertytype%ROWTYPE;
	uType egwtr_stg_usagetype%ROWTYPE;
	pSize egwtr_stg_pipesize%ROWTYPE;
	cat egwtr_stg_category%ROWTYPE;
	ptypeUsageType egwtr_stg_proptype_usage%ROWTYPE;
	ptypePipeSize egwtr_stg_proptype_pipesize%ROWTYPE;
	ptypeCategory egwtr_stg_proptype_category%ROWTYPE;
	nonMeteredRates egwtr_stg_nonmetered_rates%ROWTYPE;
	meteredRates egwtr_stg_metered_rates%ROWTYPE;
BEGIN 
	--Property Type
	for propType in (select * from egwtr_stg_propertytype order by code)
		loop 
			begin
				...
				VALUES (nextval('seq_egwtr_property_type'), propType.name, propType.code, 'Y', true, now(), now(), 1, 1);
				update egwtr_stg_propertytype set ismigrated =true where code=propType.code;		   
			EXCEPTION
			WHEN OTHERS THEN
	...
	--Usage Type
	for uType in (select * from egwtr_stg_usagetype order by code)
		loop 
			begin
				INSERT INTO egwtr_usage_type(id, name, code, active, createddate, lastmodifieddate, createdby, lastmodifiedby)
				VALUES (nextval('seq_egwtr_usage_type'), uType.name, uType.code, true, now(), now(), 1, 1);
				update egwtr_stg_usagetype set ismigrated =true where code=uType.code;		   
			EXCEPTION
			WHEN OTHERS THEN
	...
	--Pipe Size
	for pSize in (select * from egwtr_stg_pipesize order by code)
		loop 
			begin
				...
				VALUES (nextval('seq_egwtr_pipesize'), pSize.code, pSize.sizeininch, pSize.sizeinmilimeter, true, now(), now(), 1, 1);
				update egwtr_stg_pipesize set ismigrated =true where code=pSize.code;		   
			EXCEPTION
			WHEN OTHERS THEN
	...
	--cagtegory
	for cat in (select * from egwtr_stg_category order by code)
		loop 
			begin
				INSERT INTO egwtr_category(id, name, code, active, createddate, lastmodifieddate, createdby, lastmodifiedby)
				VALUES (nextval('seq_egwtr_category'), cat.name, cat.code, true, now(), now(), 1, 1);
				update egwtr_stg_category set ismigrated =true where code=cat.code;		   
			EXCEPTION
			WHEN OTHERS THEN
	...
	for ptypeUsageType in (select * from egwtr_stg_proptype_usage order by propertytype)
		loop 
			begin
				INSERT INTO egwtr_property_usage(id, propertytype, usagetype, active, createddate, lastmodifieddate, createdby, lastmodifiedby, version)
				VALUES (nextval('seq_egwtr_property_usage'), (select id from egwtr_property_type where name=ptypeUsageType.propertytype), (select id from egwtr_usage_type where name=ptypeUsageType.usagetype), true, now(), now(), 1, 1, 0);
	...
	for ptypePipeSize in (select * from egwtr_stg_proptype_pipesize order by propertytype)
		loop 
			begin
				INSERT INTO egwtr_property_pipe_size(id, propertytype, pipesize, active, createddate, lastmodifieddate, createdby, lastmodifiedby, version)
				VALUES (nextval('seq_egwtr_property_pipesize'), (select id from egwtr_property_type where name=ptypePipeSize.propertytype), (select id from egwtr_pipesize where code=ptypePipeSize.pipesize), true, now(), now(), 1, 1, 0);
				update egwtr_stg_proptype_pipesize set ismigrated =true where propertytype=ptypePipeSize.propertytype and pipesize=ptypePipeSize.pipesize;		   
			EXCEPTION
			WHEN OTHERS THEN
	...
	for ptypeCategory in (select * from egwtr_stg_proptype_category order by propertytype)
		loop 
			begin
				INSERT INTO egwtr_property_category(id, propertytype, categorytype, active, createddate, lastmodifieddate, createdby, lastmodifiedby, version)
				VALUES (nextval('seq_egwtr_property_category'), (select id from egwtr_property_type where name=ptypeCategory.propertytype), (select id from egwtr_category where name=ptypeCategory.category), true, now(), now(), 1, 1, 0);
	...
	for nonMeteredRates in (select * from egwtr_stg_nonmetered_rates order by usagetype)
		loop 
			begin
...
BEGIN
	...
	select count(*) into v_noofhousenonull from egwtr_stg_connection where houseno is null or length(trim(houseno))=0;

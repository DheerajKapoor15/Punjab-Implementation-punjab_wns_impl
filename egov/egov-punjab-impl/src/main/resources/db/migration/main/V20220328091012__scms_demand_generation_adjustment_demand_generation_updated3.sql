DROP FUNCTION IF EXISTS stms_demand_genaration(varchar,varchar,varchar,bigint);
	...
	--exempted case validation ends
			...
			if(v_demandAmountfromSWRate is not null and v_demandAmountfromSWRate > 0) then
				...
				if (v_olddemandid IS NOT NULL) then 
					...
					insert into eg_demand_details(id, id_demand, id_demand_reason, id_status, file_reference_no, remarks, amount, modified_date, create_date, amt_collected, amt_rebate) select nextval('seq_eg_demand_details'), v_newdemandid, id_demand_reason, id_status, file_reference_no, remarks, amount, modified_date, create_date, amt_collected, amt_rebate from eg_demand_details where id_demand = v_olddemandid and id_demand_reason not in (select id from eg_demand_reason where id_demand_reason_master in (select id from eg_demand_reason_master where code in('SEWERAGEADVANCE','SWTAXADJUSTMENT')));

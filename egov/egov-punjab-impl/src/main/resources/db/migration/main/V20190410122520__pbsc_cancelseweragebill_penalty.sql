CREATE OR REPLACE FUNCTION pbsc_cancelseweragebill(
...
BEGIN 
	...
	--select nextval('seq_egswtax_backend_cancelrequest') into v_seq_cancelreq;
	--raise notice 'v_seq_cancelreq %', v_seq_cancelreq;
	--insert into egswtax_backend_cancelrequest (id, consumercode, billingcycle, type, status, requestedtime) values (v_seq_cancelreq, in_consumercode, in_billingcycle, 'BILL', 'RECEIVED', now());
	...
	if(v_appldetid is null)then
		--update egswtax_backend_cancelrequest set status='FAILED', error='There is no sewerage connection with given consumer number' where id=v_seq_cancelreq;
	...
	if(v_inputinstid is null)then
		--update egswtax_backend_cancelrequest set status='FAILED', error='There is no Billing cycle defined in sewerage system for given value' where id=v_seq_cancelreq;
	...
	if(v_inputdemandid is null)then
		--update egswtax_backend_cancelrequest set status='FAILED', error='There is no demand available or bill is not generated for given billing cycle' where id=v_seq_cancelreq;
		...
		if (v_inputbillnumber is null) then 
			--update egswtax_backend_cancelrequest set status='FAILED', error='There is no active bill available for given billing cycle' where id=v_seq_cancelreq;
	...
	if (v_inputrcptavailable=true and v_nextdemandavailable=true and v_nextbillavailable=true and v_nextbillavailable=true and v_nextrcptavailable=true) then
		...
		--update egswtax_backend_cancelrequest set status='FAILED', error=v_nextrcptnumber where id=v_seq_cancelreq;
		return 0;
	end if;
	...
	if (v_inputrcptavailable=false and v_nextdemandavailable=false and v_nextbillavailable=false and v_nextbillavailable=false and v_nextrcptavailable=false) then
		...
		select dd.id into v_penaltydemanddetailid from eg_demand_details dd,eg_demand d, eg_installment_master inst, eg_demand_reason_master drm, eg_demand_reason dr where dd.id_demand = v_inputdemandid and dd.id_demand_reason = (select id from eg_demand_reason where id_demand_reason_master = (select id from eg_demand_reason_master where code = 'PENALTY' and module = (select id from eg_module where name = 'Sewerage Tax Management')) and id_installment = v_inputinstid );
		...
		--update egswtax_backend_cancelrequest set status='SUCCESS' where id=v_seq_cancelreq;

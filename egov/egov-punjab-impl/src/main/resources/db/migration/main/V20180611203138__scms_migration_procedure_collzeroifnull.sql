...
BEGIN
	v_isdatavalid := pbsc_validateData();
...
INSERT INTO egswtax_connection(id, shsc_number, status, executiondate, legacy, createdby, createddate, lastmodifieddate, lastmodifiedby, version, oldconsumernumber, zone, ward, block, locality, address)
	select v_connectionpk, v_connectionno, coalesce(status,'ACTIVE'), connectiondate, true, 1, connectiondate, connectiondate, 1, 0, oldconsumernumber, (select parent from eg_boundary where id= (select parent from eg_boundary where code=locality)), null, (select parent from eg_boundary where code=locality), (select id from eg_boundary where code=locality), addressid from egswtax_stg_connection where oldconsumernumber = oldConnectionNo;

INSERT INTO egswtax_connectiondetail(id, propertyidentifier, propertytype, noofclosets_residential, noofclosets_nonresidential, version, donationcharges, wateridentifier, usagetype, billingtype, billamount, plotsize, isplotsizeactual, ledgernumber, billgroupnumber) select v_connectiondetpk, propertyidentifier, null, resdseats, nonresdseats, 0, coalesce(securitydeposit,0), waterconnectionnumber, (select id from egswtax_usage_type where name=usagetype), billingtype, custombillamount, plotsize, (CASE WHEN isplotsizeactual = 'Yes' THEN TRUE ELSE FALSE END), ledgerid, billgroupid from egswtax_stg_connection where oldconsumernumber = oldConnectionNo;
...
declare
	...
	uType egswtax_stg_usagetype%ROWTYPE;
	sewerageRates egswtax_stg_seweragecharge_rates%ROWTYPE;
BEGIN 
	--Usage Type
	for uType in (select * from egswtax_stg_usagetype order by code)
		loop 
			begin
				INSERT INTO egswtax_usage_type(id, name, code, active, createddate, lastmodifieddate, createdby, lastmodifiedby)
				VALUES (nextval('seq_egswtax_usage_type'), uType.name, uType.code, true, now(), now(), 1, 1);
				update egswtax_stg_usagetype set ismigrated =true where code=uType.code;		   
			EXCEPTION
			WHEN OTHERS THEN
	...
	--Sewerage rates
	for sewerageRates in (select * from egswtax_stg_seweragecharge_rates order by usagetype)
		loop 
			begin
				...
				VALUES (nextval('seq_egswtax_seweragecharge_rates'), sewerageRates.ratetype, (select id from egswtax_usage_type where name=sewerageRates.usagetype), sewerageRates.fromplotsize, sewerageRates.toplotsize, sewerageRates.fromnoofseats, sewerageRates.tonoofseats, sewerageRates.effectivefromdate, sewerageRates.effectivetodate, sewerageRates.billamount, sewerageRates.fixedbillamount, now(), now(), 1, 1);
...
BEGIN
	...
	if (v_noofownernull = 0 and v_noofhousenonull = 0 and v_nooflocalitynull = 0 and v_noofpincodenull = 0 and v_noofoldconsumernumbernull = 0 and v_noofconnectiondatenull=0 and v_noofusagetypenull = 0 and v_duplicatconnection = 0) then

CREATE OR REPLACE FUNCTION pbwc_cancelwaterdemand(
...
BEGIN 
	raise notice 'Request received to cancel water demand for consumer number : % for billing cycle : %', in_consumercode, in_billingcycle;

	--logging request details
	--select nextval('seq_egwtr_backend_cancelrequest') into v_seq_cancelreq;
	--raise notice 'v_seq_cancelreq %', v_seq_cancelreq;
	--insert into egwtr_backend_cancelrequest (id, consumercode, billingcycle, type, status, requestedtime) values (v_seq_cancelreq, in_consumercode, in_billingcycle, 'DEMAND', 'RECEIVED', now());
	--raise notice 'request logged';
	select id into v_moduleid from eg_module where name = 'Water Tax Management';
	select cd.id into v_conndetid from egwtr_connection conn, egwtr_connectiondetails cd where cd.connection=conn.id and consumercode=in_consumercode;
	...
	if(v_inputdemandid is null)then
		--update egwtr_backend_cancelrequest set status='FAILED', error='The is no active demand available for given billing cycle' where id=v_seq_cancelreq;
	...
	select d.id into v_previousdemandid from eg_demand d, egwtr_demand_connection dc where dc.demand=d.id and dc.connectiondetails=v_conndetid and d.id_installment=v_previousinstid order by id desc limit 1;

	if(v_previousdemandid is null)then
		--update egwtr_backend_cancelrequest set status='FAILED', error='The connection is either migrated or created with given billing cycle' where id=v_seq_cancelreq;
	...
	select d.id into v_nextdemandid from eg_demand d, egwtr_demand_connection dc where dc.demand=d.id and d.is_history='N' and dc.connectiondetails=v_conndetid and d.id_installment=v_nextinstid order by id desc limit 1;
